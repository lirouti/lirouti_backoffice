...
| `zod` | `^4.5.4` | 위 폼의 검증 + API 응답 파싱. ⚠️ **메시지 커스터마이즈와는 무관하다**(§44.6) |
...
| `dim` | `rgba(16,24,40,.42)` | 〃 | 모달 뒤를 가리는 막 (`Dialog`). 라이트·다크가 같다 — 하는 일이 「뒤를 어둡게」 라 배경색이 무엇이든 방향이 같다 |
| `dimSoft` | `rgba(16,24,40,.32)` | 〃 | 가볍게 떴다 사라지는 것(`CommandPalette`)의 막. `dim` 보다 옅다 |
...
`bun run lint` 의 `check-contrast.ts` 가 **토큰 표 전체**를 라이트·다크 양쪽에서 잰다.
본문 텍스트 4.5:1, 큰 글씨·비텍스트 UI 3:1 (WCAG 2.1 AA), 대비는 **가장 불리한 배경** 기준.

⚠️ **Lighthouse 로는 부족하다.** 브라우저를 띄워야 하고 **그 화면에 그 색이 실제로 쓰인
경우에만** 잡힌다 — 아직 안 만든 화면의 색은 아무도 검사하지 않는다. 토큰 단계에서 재면
그 전부를 한 번에 막는다. 새 색을 추가하면 자동으로 검사 대상이 된다.

⚠️ **토큰만 잰다.** 코드에 박아 넣은 색은 이 검사가 못 본다 — 그 구멍은
`check-tokens.ts` 가 따로 막는다(§62).
...
- 탭이 둘 이상이면 스트립 오른쪽에 **`모두 닫기`**를 고정한다. 스크롤되는 탭과 함께
  밀려나면 정작 탭이 많을 때 찾을 수 없으므로 액션 영역은 스트립 밖이다. 미저장 탭이
  하나라도 있으면 개수를 알리는 확인 창을 한 번 띄운다. 확인 뒤 탭과 미저장 상태를 함께
  비우고 `/` 로 이동한다 — KeepAlive 파기는 한 틱 뒤라 cleanup 에만 맡기지 않는다.
- 가로 넘침은 가장자리 흐림뿐 아니라 **좌우 이동 버튼**으로도 드러낸다. 휠이 없는 입력
  장치에서도 숨은 탭을 찾을 수 있어야 하며, 버튼 역시 스크롤 밖의 액션 영역에 둔다.
...
- **`growth` · `emoji` 에셋** — 원본 파일에 없다. `growth` 는 **화면에서 올리면 붙는다**(§19.3 · §42.1) — 디자이너가 저장소에 넣을 필요가 없어졌다. `emoji` 는 어드민 화면 자체가 없다.
- **권한 서버 검증** — 현재 뷰어는 localStorage 기반이라 UI 게이팅일 뿐이다 (§10).
- **화면 전체를 렌더하는 테스트가 없다** — `domain/` 순수 층은 Vitest 로 덮었고(테스트 파일 24개), jsdom·Testing Library 도 들어와 조각 단위 `.test.tsx` 는 있다(`Table` · `OtpInput` · `TabBar` · `SkipLink`). 남은 것은 **화면 하나를 통째로 렌더하는 것**이고, 렌더 비용과 목 정책(react-query·라우터)이 별도 결정이라 미뤘다. 환경은 `node` 가 기본이고 DOM 이 필요한 파일만 첫 줄에 `// @vitest-environment jsdom` 을 적는다.
- **패스키(WebAuthn)** — §16.5. 비번+TOTP 를 대체하는 별도 경로로 붙인다. 비번 경로는 남긴다.
- **제재 정책** — 기간·사유·스코프가 미정이라 버튼을 잠근 채 뒀다. 손대기 전에 `nextBanStatus` 의 상태 덮어쓰기부터 볼 것 (§23.11).
...
- 테스트: **64파일 891개 통과** — `domain/` 순수 층 + 조각 단위 컴포넌트 테스트 (§60)
- 검사 일곱 (`bun run lint`, 오류 0): ESLint · 선언 순서 · 명암비 · 토큰 이름 · 표 정렬 · 주석 · 문서
- 번들 (**현재 수치의 단일 출처.** §9.2·§9.3 의 표는 결정 당시의 A/B 값이다): **첫 로드 161.31KB gzip / 예산 200KB (여유 38.69KB)** — 엔트리 + 공용 청크 + CSS. 화면은 라우트 단위 lazy 라 대시보드의 차트 청크(가장 큰 `CartesianChart` 86KB gzip 을 포함해 합 ~117KB) · `SecurityPage` 10KB gzip 은 그 화면에 들어갈 때 받는다
...
### 16.3.1 6칸을 다 채우면 바로 검증한다 — **해제만 빼고**

`OtpInput` 의 `onComplete` 가 칸이 다 찬 순간 한 번 불린다. 「확인」 을 누르지 않아도
검증이 돈다 — 6자리를 다 넣은 사람이 더 할 일은 없고, TOTP 는 30초마다 바뀌어서
누르기까지의 시간이 그대로 실패 확률이다.

| 자리 | 자동 검증 | 왜 |
|---|---|---|
| 로그인 2차 (`TotpStep`) | **한다** | 들어가려는 것이 유일한 목적이다 |
| 등록 확인 (`EnrollWizard`) | **한다** | 켜는 방향이고, 마법사의 목적이 그것뿐이다 |
| **해제** (`DisablePanel`) | **안 한다** | **끄는 동작**이다. 자리를 채우는 것으로 2단계 인증이 걷히면 「끄기」 를 누르게 한 이유(§16.3)가 사라진다 |

⚠️ **자리를 채우는 것은 「하겠다」는 표시가 아니다.** 되돌릴 수 없는 동작에는 `onComplete`
을 달지 않는다 — 마지막 한 번의 누름이 안전장치다.

네 가지가 조용히 깨질 수 있어 각각 막아 뒀다.

**① ⚠️ 값이 그대로면 `onChange` 를 부르지 않는다 — 이게 로그인을 삼킨다.**

다 찬 뒤의 7번째 숫자나 문자는 잘려서 **값이 그대로**인데, 그래도 `onChange` 를 부르면
호출부의 `reset()` 이 돈다. 그리고 **react-query 의 `reset()` 은 진행 중인 mutation 에서
observer 를 떼어낸다** — 설치된 소스가 그렇다.

```js
// @tanstack/query-core 5.102.4 · mutationObserver.js
reset() {
  this.#currentMutation?.removeObserver(this)   // ← 떼어낸다
  this.#currentMutation = void 0
  …
}
```

떼어지면 그 요청이 **성공해도 `onSuccess` 가 오지 않는다.** 자동 검증이 도는 500ms 안에
숫자를 한 번 더 누르면 **검증은 통과했는데 로그인이 안 된다.** 재현했다 — 버튼이
「확인 중…」 에서 「확인」 으로 되돌아가고 2초 뒤에도 `/login` 에 남아 있었다.

자동 검증 전에는 이 창이 「확인」 을 누른 뒤뿐이라 드물었지만, 자동 검증은 **6번째 글자가
닿는 순간** 요청을 시작하므로 빠르게 치는 사람의 7번째 타가 정확히 그 창에 들어온다.

> 잘린 글자가 DOM 에 남을 걱정은 안 해도 된다 — controlled input 은 React 가 값을
> 되돌린다. 재서 확인했다(`0000007` · `000000ab` 를 넣고도 DOM 값 `000000`).

**② ⚠️ 검증이 도는 중에는 `reset()` 하지 않는다.** ①은 값이 안 바뀌는 경우고, **실제로
바뀌는 경우**에도 같은 일이 난다(다 채운 뒤 백스페이스). 지울 오류는 그때 없으므로
(오류는 요청이 끝난 뒤에 생긴다) 호출부가 `if (!isPending) reset()` 으로 미룬다.

**③ 다 찬 뒤에 더 눌러도 다시 검증하면 안 된다.** ①의 조기 반환이 이것도 함께 막는다.
가드를 빼서 넣어 보니 추가 입력 세 번에 재검증 2회가 났다(세 번째는 ④가 막았다).

**④ 앞선 요청이 도는 중이면 겹쳐 보내지 않는다.** 코드는 일회용이라 같은 코드로 두 번
가면 뒤엣것이 실패한다. 호출부가 `isPending` 을 본다.

**⑤ 인자로 온 값을 써야 한다.** 자동 검증은 마지막 글자를 입력한 **그 이벤트에서** 불리는데,
그때 부모의 `code` 는 아직 `setState` 가 반영되기 전이다 — 그걸 읽으면 **한 글자 짧은**
코드로 검증한다. `onComplete(value)` 가 값을 넘기고, 호출부는 그것만 쓴다.

**「확인」 버튼은 남긴다.** ③의 결과로 **같은 코드를 다시 보낼 길이 자동 검증에는 없다** —
실패한 코드를 그대로 재시도하거나(시계가 막 넘어간 경우), 백업 코드를 넣는 데 필요하다.
...
### 16.6 세션이 끝나면 작업공간도 끝난다

로그아웃해도 **탭이 남아 있었다.** 탭은 `localStorage`(`riruti_admin_tabs_v2`)에 persist 되는데
`signOut` 이 `viewer` 만 비웠다.

그래서 다음 사람이 같은 브라우저에서 로그인하면 **앞 사람이 열어 두었던 화면이 그대로 보인다** —
화면 이름만이 아니라 `/items/3` 처럼 **레코드 id 까지** 남는다. 어드민에서는 그게 「누가 무엇을
보고 있었나」 를 드러낸다.

⚠️ **권한으로 가려지는 것과 지워지는 것은 다르다.** 스코프 밖 탭은 스트립에서 걸러지지만
**스토어에는 계속 남아 있다** — 실제로 운영자로 다시 로그인했을 때 `users` 탭이 화면에는 안
보이면서 `localStorage` 에는 그대로 있었다. 가려진 것을 지워진 것으로 읽으면 안 된다.

`viewerStore` 의 `endSession()` 이 `tabsStore.reset()` 과 `dirtyStore.reset()` 을 부른다.

| 부르는 곳 | 왜 |
|---|---|
| `signOut` | 사람이 누른 로그아웃 |
| `clear` | **401 로 세션이 끊긴 경로.** 여기만 빠뜨리면 세션 만료로 튕긴 뒤 재로그인했을 때 탭이 살아 있다 |

⚠️ **`preview`·`exit` 에서는 부르지 않는다.** 미리보기는 **보이는 범위만** 바꾸는 것이고 로그인한
사람은 그대로다 — 거기서 탭을 닫으면 하던 일이 사라진다.

⚠️ **`dirtyStore` 도 함께 비운다.** persist 하지 않지만 로그아웃은 새로고침이 아니라, 비우지 않으면
새 세션의 탭에 앞 세션의 점(●)이 붙는다.
...
**DOM 이 필요하면 파일 첫 줄에 `// @vitest-environment jsdom`.** 전역 환경은 `node` 이고,
그 이유와 컴포넌트 테스트의 경계는 §60 에 있다.
...
말하고, 필드 오류는 제출을 시도해 `tried` 가 true가 된 뒤에 붙는다.
...
개수가 기획으로 고정된 집합에 **등록 버튼을 붙이지 않는다.** 운영자가 눌렀을 때 게임이
감당 못 하는 데이터가 생긴다 — 둥지를 4번째로 만들면 해금 일수 구간이 겹치고, 성장 단계를
늘리면 클라이언트에 그 연출이 없다.

⚠️ **그렇다고 내용까지 고정인 것은 아니다.** 한동안 이 규칙이 세 가지를 뭉쳐 놓고 있었다 —
등록 화면이 없으니 업로더가 붙을 자리도 없어져서 **그림 교체와 기준치 수정까지 같이
막혔다.** 4단계가 고정인 것과 「알」 그림이 고정인 것은 다른 말이다. 디자이너가 알을 다시
그리면 그건 새 줄이 아니라 **같은 줄의 교체**다.

| | 줄 추가 | 그림 교체 | 기준치 수정 |
|---|---|---|---|
| 성장 4단계 | ❌ | ✅ 업로드 | ✅ 경계 둘 (§42.3) |
| 둥지 3단계 | ❌ | ✅ 업로드 | ✅ 경계 둘 (§41.4) |
| 리그 슬롯 6 | ❌ | ❌ | ❌ |

⚠️ **리그만 예외다.** 실루엣(`rg`)과 앵커 좌표는 다른 모든 아트가 맞춰야 하는 **기준**이라,
바뀌면 코드와 아트가 함께 움직여야 한다. 여기만 빌드 경로(`design/` + `bun run assets`)에
남긴다 — 그래서 빌드 경로는 **아이콘 · 리그 · 초기 카탈로그 씨앗**으로 좁아졌고, 운영 중
들어오는 그림은 전부 업로드로 일원화된다(§8.5).

### 19.3.1 ⚠️ 경계만 저장하고 표시는 파생한다

기간에서 나오는 값을 저장하지 않는 것은 공지·챌린지에서 이미 세 번 정한 규칙이다
(§25.1 · §20.2). 성장과 둥지도 같다 — 그런데 여기는 **원본이 표시용 문자열을 그대로 들고
있어서** 더 위험했다.

```text
성장   '0–2일' · '부화 직전' · '3–13일' · '14일~'
둥지   sub: '누적 1–29일 · 성글게 엮인 첫 둥지'  ·  cond: '누적 1–29일'   ← 같은 날짜가 두 칸에
```

경계 하나를 옮기면 **두 줄의 표시가 함께** 바뀌어야 하는데, 문자열로 들면 하나만 고치고
끝난다 — 둥지는 카드와 표가 서로 다른 일수를 말하게 된다. 그래서 저장하는 것은
`fromDay` 하나이고 보이는 문구는 `stageSpan`·`unlockLabel` 이 만든다.

**실제로 고치는 것은 각각 경계 둘뿐이다.** 첫 칸은 언제나 0일차(알)·1일차(첫 둥지)부터라
경계가 아니다.

```text
성장   알 ──┬── 유체 ──┬── 성체        경계 3일 · 14일
둥지   잔가지 ┬ 튼튼한 ┬ 보금자리        경계 30일 · 100일
```

### 19.3.2 ⚠️ `assetId` 는 식별자가 못 된다

그림 교체를 열면서 `assetId` 가 **바뀌는 값**이 됐다. 그런데 처음에는 그걸 그대로 줄
식별자로 썼다 — React 키로도, 파사드의 조회 키로도.

**둥지 셋이 카탈로그의 셋을 그대로 쓰고 있어서 겹침이 거의 확정이다.** 다른 그림을 고르는
순간 두 줄이 같은 id 를 갖는다.

```text
① 보금자리 ← 잔가지의 그림      두 줄이 as_nest_0
② 보금자리를 또 바꾼다           assetId 로 찾으면 **잔가지**가 먼저 잡힌다 → 엉뚱한 줄이 바뀐다
```

그래서 **줄마다 고정 `key` 를 둔다.** 늘지도 줄지도 않는 집합이라 값이 바뀔 일이 없다.

⚠️ **한 번짜리 테스트로는 못 잡는다.** 처음 교체할 때는 `assetId` 가 아직 유일해서 맞는
줄이 잡히고, **겹치고 난 뒤의 다음 교체**가 앞줄을 고친다. 목 테스트를 두 단계로 쓴 이유다
(`mocks/nests.test.ts`).
...
### 20.5.2 보상은 아이템을 **가리켜야** 한다

보상이 `assetId` + 이름만 들고 있었다. 그러면 **어느 아이템인지 물을 수 없다** — 상세를
열 수도, 「지워졌는가」 를 알 수도 없다. §20.4 가 적어 둔 대로 **그림만 같고 가격·획득
경로가 다른 아이템이 여럿일 수 있어서** 이름으로 되짚는 것도 믿을 수 없다(지금 목에
중복이 없을 뿐이다).

`itemKey` 를 싣는다. 성장·둥지에서 밟은 것과 **같은 함정**이다(§19.3.2) — 바뀌거나 겹칠
수 있는 값을 식별자로 쓰면 조용히 엉뚱한 것을 가리킨다.

**고른 줄 판정도 키로 한다.** 예전에는 `assetId` 와 이름을 함께 견줬다.

⚠️ **목 씨앗도 같은 규칙을 지켜야 한다.** 규칙을 세운 그 커밋에서 `mocks/challenges.ts`
가 `allItems()` 를 **`assetId` + 이름으로 훑어** 아이템을 찾고 있었다 — **금지한 조회를
금지한 자리에서 쓴 것**이다. 게다가 `allItems()` 와 `allChallenges()` 가 **둘 다 게으른
캐시**라 순서가 결과를 바꾼다.

```text
실측 — 챌린지가 만들어지기 전에 아이템 하나를 개명
  assetId + 이름으로 찾기   보상 10건 중 7건만 남았다   3건이 조용히 사라졌다
  슬롯 안 자리로 집기        10건 그대로
```

씨앗은 **슬롯 안 몇 번째**로 집는다. 아이템이 슬롯 순서대로 만들어지므로 자리가 곧
아이템이고, 이름이 바뀌어도 같은 것을 가리킨다. 반환하는 `slot` 도 인수가 아니라
**아이템의 것**을 쓴다 — 둘이 갈리면 키와 슬롯이 서로 다른 아이템을 가리킨다.

#### 20.5.2.1 키를 실었어도 **베껴 둔 값은 그대로 남는다**

`ChallengeReward` 는 `itemKey` 와 함께 `assetId`·`assetSrc`·`name`·`slot` 을 **베껴서**
들고 다닌다. 목록 50줄을 그리자고 아이템 50개를 따로 받을 수는 없기 때문이다.

⚠️ **그래서 아이템 이름이 바뀌면 한 화면 안에서 두 값이 보인다.** 폼의 보상 칸은 베껴 둔
이름을, 거기서 연 상세 창은 **키로 받아 온 현재 이름**을 보여 준다.

지금은 **그대로 둔다.** 상세 창의 일은 「이 아이템이 맞는가」 를 답하는 것이라 현재 값이
맞고, 둘이 다르게 보이는 것 자체가 **「고르고 나서 아이템이 바뀌었다」 는 신호**다.
베껴 둔 값을 읽을 때마다 새로 맞추려면 목록이 줄마다 조회를 하게 된다.

TODO(보상 표시가 틀렸다는 문의가 오면): 저장 시점에 베낀 값을 다시 채우는 쪽이 먼저다 —
읽을 때가 아니라 **쓸 때** 맞추면 조회가 늘지 않는다.

### 20.5.3 고르기 전에 무엇을 주는지 본다

고르기 창은 이름·슬롯·코드만 보여 준다. 그래서 **미노출 아이템이나 유료 아이템을 보상으로
거는 것**을 눌러 보기 전에는 모른다 — 챌린지를 만들고 나서야 드러난다.

줄마다 「상세」 를 두고, **폼에서 이미 고른 것도 다시 볼 수 있게** 했다(수정으로 들어오면
이름만 보인다).

⚠️ **아이템 화면으로 보내지 않는다.** 작성 중인 챌린지를 두고 탭을 옮기면 하던 일을
놓친다. 여기서 확인하려는 것은 「이 아이템이 맞는가」 지 판매 추이가 아니라, 창으로 겹쳐
띄우고 필요한 것만 보여 준다 — 노출 상태 · 등급 · 가격 · 획득 경로.

⚠️ **가리키던 아이템이 사라졌으면 그렇게 말한다.** 빈 칸을 두면 「안 불러왔다」 와 구분이
안 된다 — 이벤트 보상과 같은 규율이다(§25.2).

**못 보여 주는 이유를 셋으로 가른다.** 한때 어떤 오류든 「보상을 다시 고르세요」 라고
했는데, 그러면 **잠깐 끊긴 네트워크에 대고 멀쩡한 보상을 버리라고** 하게 된다.

| 무엇 | 문구 | 운영자가 할 일 |
|---|---|---|
| 404 | 「…찾을 수 없습니다. **보상을 다시 고르세요**」 | 다시 고른다 |
| 그 밖의 오류 | 「…**창을 닫았다 다시 열어 주세요**」 | 기다렸다 다시 본다 |
| `itemKey` 없음 | 「아이템을 **가리키고 있지 않습니다**」 | 다시 고른다 |

셋째 줄은 **서버가 예전 모양으로 줄 때**를 위한 것이다(`itemKey` 는 나중에 생겼다,
§20.5.2). 지금 경로로는 닿지 않는다 — 초안은 `sameShape` 가 키 집합을 보고 거른다(§33.1).
그래도 두는 이유는 본문을 비우면 「안 불러왔다」 와 똑같아 보이기 때문이다.

⚠️ **스켈레톤은 `SkeletonRows` 가 아니다.** 본문이 132px 그림 + 배지줄 + 제목 + 6줄인데
글줄 넷으로 덮으면 **도착하는 순간 창이 커지면서 닫기 버튼이 손가락 밑에서 움직인다.**

```text
실측 — 들여다보기 창 본문 213px
  SkeletonRows rows={4}   172px   → 41px 튄다
  본문 모양 스켈레톤       208px   →  5px
```

**고르기 창에서는 스켈레톤이 아예 안 뜬다.** 목록이 이미 여섯 칸을 다 갖고 있어서
아이템을 그대로 넘긴다 — 화면에 떠 있는 값을 다시 받아 오며 깜빡일 이유가 없다.
받아 오는 것은 **폼에서 이미 고른 것을 다시 볼 때**뿐이다(거기는 이름·슬롯만 갖고 있다).
...
목록의 「보유 재화」 열은 보석 표식과 함께 `유상 1,840` / `무상 320`을 두 줄로 쓴다.
색만으로 구분하지 않고 라벨을 먼저 보여 주며, 숫자는 같은 색으로 우측 정렬해 비교한다.
...
> **폼 5곳이 이 규약을 어기고 있었다.** 아이템·업적·배경·챌린지·종류의 제출 버튼을
> `blocked` 로 잠가 두어서, 무엇이 왜 남았는지 말하지 못했다. 잠금을 풀고 **누르면 그
> 자리에서 말하도록** 바꿨다 (§44.4).
...
**둘은 서로 다른 시점을 본다** — AI 는 인증이 **올라올 때** 심사하고, 신고는 **올라온 뒤**에 들어온다. 그래서 한 내비 그룹이지만 규칙이 섞이지 않는다.

### 23.0 신고는 가리지 않고 줄 세운다

**예전에는 신고 누적이 기준치를 넘으면 자동으로 숨겼다.** 지금은 숨기지 않는다 — 「기준 초과」 를 달아 큐 최상단으로 올릴 뿐이고, **사진을 내리는 것은 관리자의 조작 하나뿐**이다.

⚠️ **이건 개선이 아니라 맞바꿈이다.** 어느 쪽 사고를 감수할지가 바뀔 뿐 사고가 없어지지 않는다.

| | 기존 (자동 숨김) | 지금 (줄 세우기) |
|---|---|---|
| 사고 | 오신고 몇 건으로 정상 인증이 가려진다 | 문제 사진이 관리자가 볼 때까지 계속 보인다 |
| 피해자 | 인증을 올린 회원 | 그 사진을 보는 다른 회원 |
| 되돌리기 | 숨김을 풀면 원상복구 | **불가능** — 이미 본 사람은 봤다 |
| 기본값 | 의심스러우면 가린다 | 의심스러워도 보여 준다 |

⚠️ **그래서 태그(§23.3) · 최상단 정렬(§23.4) · 사이드바 배지(§23.6) 셋은 한 묶음이다.** 자동 숨김이 늦은 대응을 대신 막아 주던 몫을, 지금은 **관리자가 빨리 보는 것**이 전부 감당한다. 하나라도 빼면 이 변경은 「보호 장치를 뗐다」 가 된다.

**상태 이름도 함께 바꿨다.** 옛 이름은 일어나지 않는 일을 설명한다 — 해제할 숨김이 애초에 없으니 「숨김 해제」 라고 부를 수 없고, 「대기」 도 무엇을 기다리는지가 바뀌었다(가려진 채가 아니라 **보이는 채로** 기다린다).

```text
대기 · 숨김 유지 · 숨김 해제  →  미검토 · 노출 유지 · 숨김
```

신고 하나는 원래 축이 둘이다 — **사진이 보이는가**와 **관리자가 봤는가**. 자동 숨김이 있을 때는 둘이 붙어 다녀 값 셋으로 덮였는데, 떼어 내도 **빈 칸이 옮겨 갈 뿐 여전히 셋**이라 필드를 쪼갤 이유가 없다.

| 기존 | 미검토 | 검토 완료 |
|---|---|---|
| 숨김 | 대기 | 숨김 유지 |
| 노출 | *자동으로 가려지므로 없다* | 숨김 해제 |

| 지금 | 미검토 | 검토 완료 |
|---|---|---|
| 노출 | 미검토 | 노출 유지 |
| 숨김 | *숨기는 건 관리자뿐이므로 없다* | 숨김 |

**버튼 둘이 대칭이 됐다.** 예전 「숨김 유지」 는 **그것만 아무것도 안 바꾸는 조작**이라 누를 이유를 설명할 수 없었다(이미 숨겨진 것을 숨긴 채로 둔다). 지금은 「숨김」 이 사진을 실제로 내리고 「노출 유지」 가 검토 종료를 기록한다.

⚠️ **감사 로그의 `isUnchanged`(§32.4)가 걸리는 대상이 정확히 뒤집혔다.** 규칙(`from === to`)은 그대로다.

| 조작 | from → to | 화살표 |
|---|---|---|
| 기존 「숨김 유지」 | 숨김 → 숨김 | 안 그린다 |
| 지금 「숨김」 | **노출 → 숨김** | **그린다** |
| 지금 「노출 유지」 | 노출 → 노출 | 안 그린다 |
...
**처리하면 다음 건으로 옮겨 간다** (`nextAfterRemoved`). 「미검토」 탭에서 처리한 행은 목록에서 사라지는데, 그때 아무것도 안 고르면 오른쪽이 빈 화면이 되어 **매번 다음 건을 손으로 눌러야 한다.** 뒤 행을 먼저 고르고 마지막이었으면 앞 행으로 간다.

> 「검토 완료」·「전체」 탭에서는 행이 그대로 남으므로 옮기지 않는다. 안 그러면 방금 처리한 결과를 확인할 새도 없이 화면이 바뀐다.

**이미 그 상태인 버튼은 잠근다.** 「숨김」 인 건에 다시 「숨김」 을 누르면 아무 일도 안 일어나는데 버튼은 반응한 것처럼 보인다. 반대 결정은 막지 않는다 — 잘못 내린 건을 되돌리는 것이 이 화면의 목적이다.
...
```text
「숨김」 누름 → 60ms 뒤 「전체」 탭으로 이동 → ?tab=전체
                  → 250ms 뒤 변이 성공, 낡은 params 를 다시 씀
결과              → ?id=1  ·  탭이 「미검토」 로 되돌아감
```
...
### 23.3 기준치는 처분이 아니라 신호다

`REPORT_THRESHOLD = 5`. 이 수 이상 쌓이면 「기준 초과」 배지를 달고 큐 맨 위로 올린다.

**고정이 소수여야 고정이다.** 목 8건의 신고자 수는 `3 · 5 · 4 · 6 · 3 · 7 · 4 · 3` 이다.
...
3으로 낮추면 **목록 전체에 표시가 붙어 아무것도 구별하지 못하고**, 4도 절반이 넘는다. 반대로 6은 신고 5건짜리를 평범한 건으로 떨어뜨린다.

- ⚠️ **단조 증가다.** 신고를 취소하는 경로가 없어서 한 번 넘으면 계속 넘는다 — 「기준 초과」 는 시점이 아니라 **누적된 사실**이다.
- ⚠️ **검토를 마쳐도 태그는 남긴다.** 신고가 많았다는 건 관리자의 판단과 무관한 사실이라 「노출 유지」 로 닫은 건에서도 지우지 않는다. 다만 **최상단 고정은 미검토에만** 건다(§23.4).
- **배지에 건수를 넣지 않는다.** 바로 옆에 「신고 7건」 이 있어서 「기준 초과 7건」 으로 쓰면 같은 수를 두 번 세는 것처럼 읽힌다.
- **색만으로 말하지 않는다.** 글자가 「기준 초과」 라서 색을 못 보는 사람에게도 뜻이 전달된다.

> TODO(운영 설정 API 가 생기면): `ops` 설정 테이블의 한 행으로 뺀다. 지금은 도메인 상수다.

### 23.4 정렬은 도메인이 한다

```text
1  미검토 · 기준 초과      ← 여기만 최상단으로 끌어올린다
2  미검토
3  검토 완료 (숨김 · 노출 유지)

각 묶음 안: 신고 많은 순 → 최근 순 → key (동률 고정)
```

**화면이 정렬하면 탭마다 순서가 갈린다.** 지표도 거르기 전 전체로 내야 해서(§23.1) 파사드가 어차피 전량을 쥐므로, `sortReports` 를 거쳐 내려보낸다.

- ⚠️ **`Array#sort` 는 제자리 정렬이다.** 목 캐시 배열을 그대로 넘기면 **다음 조회의 순서까지 바뀐다** — §25.2 에서 이미 밟은 함정이라 복사하고 정렬한다.
- ⚠️ **검토가 끝난 건은 기준을 넘겼어도 올리지 않는다.** 판단이 끝나 급할 것이 없고, 올리면 「먼저 볼 것」 신호가 희석된다.
- ⚠️ **CSS `order` 로 끌어올리지 말 것.** 시각 순서만 바뀌고 DOM 순서는 그대로라, 스크린리더와 키보드 탭 순서에는 우선순위가 없는 목록이 된다.

**지표 넷도 정책을 따라 바뀌었다.** 「우선 검토」 는 **「미검토」 의 부분집합**이라 둘을 더하면 안 된다 — 라벨에 기준치(`신고 5건+`)를 적어 두 칸이 같은 것을 다르게 세고 있음을 드러낸다.

| | 세는 것 |
|---|---|
| 우선 검토 · 신고 5건+ | 기준 초과 **&** 미검토. 이 화면에 온 이유라 첫 칸이고 `alert` 다 |
| 미검토 | 우선 검토를 포함한 전체 대기량 |
| 오늘 접수 | 유입 속도. 상태와 무관하게 날짜에서 센다(§21.4) |
| 숨김 | 관리자가 실제로 내린 건수 |
...
**이 자리는 `entities/moderation/CertPhotoPanel` 이다.** AI 심사의 「보기」 도 같은 것을 쓴다 — 보는 시점은 다르지만 취급 규칙은 같다 (§66).

### 23.6 사이드바 배지 — 셸은 데이터를 모른 채로 둔다

「신고 처리」 옆 숫자는 **기준 초과 미검토 건수**다. `nav.ts` 에 `count: 5` 가 박혀 있던 자리인데, **새 표시를 더하기보다 이미 거짓말을 하던 자리를 사실로 만드는 쪽이 먼저**라서 여기부터 손봤다.

⚠️ **`NavChild.count` 와 다른 것을 센다.** `count` 는 「그 화면에 항목이 몇 개 있는가」 라 늘 같은 값이고, `NavAlerts` 는 「그중 몇 개가 나를 기다리는가」 다. 그래서 색도 다르다(회색 대 위험색). **`0` 은 배지를 아예 안 그린다** — 「0건이 기다린다」 는 알릴 것이 없다는 뜻이다.

**배선** — `layouts` 는 `api` 를 부를 수 없다(§4.3). 로그아웃과 같은 방향으로 뒤집되, 이번에는 핸들러 주입이 아니라 **조립 지점이 값을 넘긴다.**

```text
app/AdminShell  ──(alerts)──▶  AdminLayout ▶ Sidebar
      │
      └─ api/moderationAlerts   (useReports 와 **같은 쿼리 키**)
```

키가 같아서 **한 건을 처리하면 무효화 한 번으로 배지와 목록이 함께 갱신된다** — 둘이 어긋날 수 있는 경로가 없다. 재 봤다: 「숨김」 한 번에 배지 2 → 1, 「우선 검토」 2 → 1 · 「미검토」 5 → 4 · 「숨김」 1 → 2, 큐 5행 → 4행.

⚠️ **화면이 열려 있지 않아도 돌아야 한다.** 「신고 처리」 탭을 열어야만 배지가 맞는다면 그건 알림이 아니다 — 이미 보고 있는 사람에게만 알리는 셈이다. 그래서 `features` 가 아니라 셸이 부른다.

⚠️ **`api/moderationAlerts` 를 따로 둔 것은 번들 때문이다.** 셸은 eager 라, 신고 파사드를 정적으로 `import` 하면 목 데이터까지 엔트리 청크에 들어가 **로그인 화면에서도 받는다.** 예산 검사(§9.4)가 잡아낸 실제 사고다.

| 첫 로드 gzip | |
|---|---|
| 배지 없음 | 159.02KB |
| 정적 `import` | 164.50KB (+5.48) |
| 동적 `import` (지금) | 161.25KB (+2.23) |

남은 2.23KB 는 `useQuery` 자체다 — 셸이 쿼리를 구독하는 값이라 뺄 수 없다. 배지는 첫 페인트에 필요한 값이 아니므로 `queryFn` 안에서 미룬다.

> **알림을 여기까지만 한 이유** — 화면 안에서 알릴 자리는 이미 셋(태그 · 최상단 고정 · 배지)이라 배너를 더하면 같은 것을 네 번 말한다. 정말 필요한 것은 「어드민을 안 보고 있을 때 알려 주는 것」 인데 그건 메일·슬랙이고 **발송 주체가 서버**다. ⚠️ 서버가 안 보내는데 화면에 「알림 발송됨」 을 적으면 안 된다(§23.5 와 같은 규율).

### 23.7 사유는 선택지 넷 + 「기타」 다

앱의 신고 폼은 **정해진 문구 넷 중 하나를 고르게 하고, 「기타」 를 고른 사람에게만 `textarea` 를 연다**(최대 500자, `REASON_DETAIL_MAX`). 성질이 다르므로 그리는 법도 나눈다.

| | 정해진 문구 넷 | 「기타」 본문 |
|---|---|---|
| 길이 | 고정 — 최대 9자 | 제각각 — 최대 500자 |
| 어휘 | 닫혀 있다 (셀 수 있다) | 열려 있다 (사람이 쓴 글) |
| 그리는 법 | **배지** | **인용 블록 · 세 줄에서 접기** |

**배지가 남는 이유는 신고자 일곱을 훑을 수 있어야 하기 때문이다** — 같은 사유가 반복되는지가 판단의 재료인데, 문장으로 늘어놓으면 그게 안 보인다.

⚠️ **본문을 배지로 그리면 안 된다.** `Badge` 는 `white-space: nowrap` 이라 문장 하나가 한 줄로 늘어나 카드를 밀어낸다. 신고자 카드는 `flex: 1 1 280px` 자리라 500자짜리 하나가 레이아웃을 무너뜨린다.

⚠️ **`why !== '기타'` 면 `detail` 은 빈 문자열이다.** 옵셔널로 두지 않은 이유는 「안 왔다」 와 「비어 있다」 를 화면이 구분할 필요가 없어서다 — 둘 다 그릴 것이 없다.

**접기 여부를 글자 수로 어림하지 않는다.** 처음엔 「60자를 넘으면 「전문 보기」 를 붙인다」 로 짰다가 되돌렸다. 카드 폭이 `flex` 라 **한 줄에 들어가는 글자 수가 창 크기마다 달라서**, 세 줄에 딱 맞는 글에도 눌러 봐야 아무 일 없는 버튼이 붙는다 — §18.8 이 따로 금지해 둔 종류다. `ResizeObserver` 로 실제 넘침(`scrollHeight > clientHeight`)을 잰다.

| 브라우저에서 잰 값 | 접힘 | 실제 | 버튼 |
|---|---|---|---|
| 118자 | 62px | 78px | 전문 보기 |
| 48자 (넓은 창) | 46px | 46px | 없음 |
| 48자 (좁은 창 · 세 줄) | 62px | 62px | 없음 |

> **펼친 동안에는 재지 않는다** — 클램프가 풀려 있어 `scrollHeight === clientHeight` 라, 재면 「안 넘쳤다」 가 되어 접을 버튼이 사라진다. keep-alive 가 DOM 을 떼었다 붙여도 관찰자가 다시 재므로 `useEffectOnActive` 가 필요한 종류의 구독이 아니다(도는 타이머가 없다).

### 23.8 AI — 원본의 차트는 자기 통과율을 설명하지 못했다
...
### 23.11 제재는 아직 정하지 않았다

「이 회원 제재하기」 는 `disabled` 로, 라벨에 「준비 중」 을 적은 채 둔다(§18.8).

⚠️ **정할 때 먼저 볼 것** — `domain/user/rules.ts` 의 `nextBanStatus` 가 `status` 를 `BANNED` 로 **덮어써서 원래 상태를 잃는다.** 휴면이던 사람의 제재를 풀면 「정상」 이 되어 되살아난 것처럼 읽힌다. 관리자 계정에서 이미 같은 문제를 풀어 뒀다 — §31.3 「정지는 상태가 아니라 덮개다」. 기간·사유를 어떻게 받든 **그 구조 위에 얹힌다.**

함께 정해야 하는 것들: 기간(영구 / 기간제 — 기간제면 §25.1 대로 **날짜를 저장하고 상태는 파생**), 사유 필수 여부(§22.4 의 환불 사유와 같은 성격), **제재와 숨김이 별개 조작**이라는 점(회원을 막아도 올라간 인증은 그대로 보인다), 스코프(`mod` 만 있는 운영자가 회원 상태를 바꿀 수 있는가).

⚠️ **자동 제재는 만들지 않는다.** 기준치는 보라는 신호이지 처분이 아니다(§23.3) — 자동 숨김을 걷어낸 마당에 자동 제재를 넣으면 **더 약한 조치를 사람 손에 넘기면서 더 강한 조치를 기계에 맡기는** 셈이 된다.
...
### 24.1.1 한동안 「변경 저장」 이 없었다

원본에는 「CSV 가져오기」 와 「변경 저장」 두 버튼이 있는데 **표에 입력 칸이 하나도 없었다.** 바꿀 수단이 없으면 저장할 것도 없어서 「변경 저장」 을 지웠다 — 원본의 가짜 검색창을 지운 것과 같은 판단이다(§18.8 의 반대편 경우다).

그때는 **CSV 로 통째로 갈아 끼우는 것이 실제 편집 방식**이라고 보고, 「행 단위 저장 API 를 먼저 만들면 쓰이지 않는다」 고 적어 두었다.

⚠️ **그 전제가 바뀌어 행 단위 편집을 열었다**(§24.1.2). 운영자가 한 줄씩 손보는 쪽을 쓰기로 했다. **「CSV 가져오기」 는 그대로 잠긴 채 남는다** — 시즌 개편처럼 통째로 바꾸는 일은 한 줄씩 할 것이 아니라, 둘은 서로를 대신하지 않는다.

### 24.1.2 고치면 「검수 중」 으로 내려간다

이 표의 숫자는 **이미 그 레벨에 있는 회원에게 적용된다.** 그래서 상태가 있고, 릴리스 전에 기획이 확인한 행만 「적용」 이다. **고친 채로 「적용」 이 남으면 검수를 건너뛴다** — `applyLevelEdit` 이 저장할 때 자동으로 내린다.

⚠️ **경험치를 고치면 아래 모든 행의 누적이 움직인다.** `total` 은 `need` 의 러닝 합이라(§24.1) 한 칸을 고쳐도 표 절반이 바뀐다. **저장 전에 그 값을 보여 준다** — 입력 칸만 바꿔 두면 운영자는 한 칸을 고쳤다고 생각하고 누른다.

움직인 행에는 `↕` 를 붙이고 색을 준다. **색만으로 말하지 않는 것**이 규약이라 기호를 함께 쓴다.

⚠️ **미리보기는 도메인의 같은 함수로 만든다.** 화면이 따로 계산하면 **저장 결과와 갈린다** — 보여 준 것과 저장된 것이 다르면 미리보기가 아니라 거짓말이다(§31.4 와 같은 규율).

| | |
|---|---|
| 필요 경험치 | 1 이상 정수. **0 이면 그 레벨을 경험치 없이 지나간다** — 표에서는 누적이 안 는 것으로만 보여 알아채기 어렵다 |
| 젬 보상 | **0 을 허용한다.** 보상이 없는 레벨은 기획이 실제로 두는 값이고, 「없음」 이 아니라 정말 0 개다 |
| 해금 | 비울 수 없고 40자까지. 표 한 칸에 들어와야 한다 |

### 24.1.3 ⚠️ 빈 칸과 `0` 은 다른 것이다

폼 초안을 **숫자로** 들고 있었다. `Number('')` 는 `0` 이라, **젬 보상처럼 0 이 유효한
칸에서는 칸을 비운 순간 멀쩡한 값이 되어 저장 버튼이 열린다** — 실제로 60 이 0 으로
바뀌었다. 게다가 통제된 입력이라 지운 칸이 곧바로 `0` 으로 다시 그려져, **비울 수도 없다.**

그래서 초안은 **날것 문자열**로 든다(`LevelDraft`). 빈 칸은 값이 아니라 **아직 안 적은
것**이고, 검증이 그렇게 말한다.

```text
지웠다   → 칸은 비어 있고  「젬 보상을 입력해 주세요」   저장 잠김
-5       → 친 글자 그대로  「0 이상의 정수여야 합니다」  저장 잠김
0        → 통과
```

⚠️ **`parseCount` 를 쓰지 않는다.** 그 헬퍼는 빈 칸도 못 읽은 글자도 전부 `0` 으로 눌러
담는다(§59.6 — 통제된 입력에서 `NaN` 으로 칸이 깨지는 것을 막으려는 결정이고, 개수·금액
칸에서는 맞다). **여기서는 그 눌러 담기가 위험을 만든다.** 밸런스 상수는 빠르게 치는 값이
아니라, 친 글자를 그대로 두고 무엇이 틀렸는지 말해 주는 편이 맞다.

> 같은 함정이 **0 이 유효한 다른 칸**에도 있다 — 상점의 `bonus` 가 그렇다. 거기도 폼이
> 숫자로 들고 있으면 지웠을 때 0 이 된다. 이 표만의 문제가 아니라는 뜻이다.

⚠️ **형식 오류와 값 오류가 같은 하한을 말해야 한다.** 필요 경험치는 1 이상인데 형식
메시지만 「0 이상」 이라 **같은 칸이 무엇을 치느냐에 따라 다른 하한**을 말했다 —
`abc` 에는 「0 이상」, `0` 에는 「1 이상」. 검증이 둘로 갈린 자리에서 나기 쉬운 어긋남이라
테스트로 고정했다.

⚠️ **빈 칸 오류가 값 오류를 덮어야 한다.** 빈 칸에 「1 이상이어야 합니다」 라고 하면
무엇을 하라는 말인지가 달라진다. 빈 칸은 자리끼움 값으로 치환해 값 검증에 넘기고,
메시지는 빈 칸 쪽이 이긴다 — **둘 다 있어야** 한쪽이 무너져도 딴소리가 안 나온다.
...
「노출 유지」 는 **살펴보고 그대로 두기로 한** 조작이다. 원본은 이것도 빨강 → 초록 화살표를
그렸다. 색과 화살표가 「바뀌었다」 를 말하는데 값은 같아서, **한 번 더 읽어야 안 바뀐 걸
안다.**

`isUnchanged` 면 화살표 대신 한 칸으로 적는다 — **「노출 · 그대로 유지」**.

⚠️ **자동 숨김을 걷어내면서 이 규칙이 걸리는 대상이 정확히 뒤집혔다**(§23.0). 규칙
자체(`from === to`)는 그대로인데, 예전에 안 바뀌던 쪽(「숨김 유지」)이 지금은
**노출 → 숨김으로 실제로 사진을 내리는** 조작(「숨김」)이 됐다. **규칙을 고치지 말고
예시를 고칠 것** — 테스트도 그렇게 옮겼다.
...
| ⚠️ **너무 본다** | **표본이 `null` 인 자리는 `null` 만 받는다** — 아래 |

#### 33.1.1 ⚠️ 「키 집합이 같은가」 가 **선택 필드와 부딪친다**

두 번 데었고 둘 다 **조용히 초안이 사라지는** 모양이었다.

**① `undefined` 인 선택 필드는 왕복에서 키가 사라진다.** `JSON.stringify` 는
`{assetSrc: undefined}` 를 그냥 빼므로, 표본은 5키인데 읽은 것은 4키가 되어 **키 개수
비교에서 탈락한다.** 챌린지 보상에 `assetSrc` 를 무조건 실었다가 **보상 있는 챌린지의
초안이 전부 복원에 실패했다.** → 없으면 **키 자체를 넣지 않는다**(`...(v ? {k: v} : {})`).

**② 표본이 `null` 이면 그 자리는 `null` 만 통과한다.** `sameShape(v, null)` 이
`v === null` 이라서다. 빈 배열 표본을 「비교할 것이 없으니 못 본다」 로 다루는 것과
**정반대**인데, 상황은 같다 — 표본이 알려주는 것이 없다.

```text
실측 — 챌린지 등록에서 보상 아이템을 고른 뒤의 초안
  표본 = emptyChallengeInput() → rewardItem: null
  초안 = { …, rewardItem: { itemKey, assetId, name, slot } }   ❌ 통째로 버려진다
```

**보상을 고르는 순간 그 폼의 초안 전체가 못 쓰게 된다** — 제목도 조건도 같이 날아간다.
수정 화면은 표본에 보상이 들어 있어 멀쩡하고, **등록 화면만** 그렇다.

TODO(고치기로 하면): `null` 표본을 「이 자리는 nullable 이고 비어 있을 때는 못 본다」 로
다루면 풀린다 — 빈 배열과 같은 규칙이다. 다만 **모든 폼에 걸리는 변경**이라 이 자리에서
같이 고치지 않았다. 고칠 때는 `refine` 으로 값 제약을 받아 두는 편이 낫다.
...
### 35.2 헤더는 본문과 같은 정렬을 쓴다

**열의 값 종류가 정렬을 정하고, 제목은 제 데이터 위에 선다.**
...
⚠️ **`<th>` 의 브라우저 기본값은 `center` 다.** 그래서 좌측 열도 **명시적으로** `left` 를
적는다. 비워 두는 것이 곧 어긋남이다.

한동안은 「헤더는 열 종류와 무관하게 항상 중앙」 이었다. 근거는 *"짧은 한국어 열 제목이
좌·중·우로 섞이면 헤더 띠가 울퉁불퉁하게 읽힌다"* 였는데, **넓은 열에서 대가가 훨씬 컸다.**
AI 심사 목록을 재 보니 이랬다.
...
일시     150px     중앙  ←→ 좌측
회원     110px     중앙  ←→ 좌측
챌린지   599.7px   중앙  ←→ 좌측   ← 제목이 데이터에서 300px 떨어져 있었다
소요      80px     중앙  ←→ 우측
```

헤더 띠가 고르다는 것과 **제목이 제 데이터를 가리킨다**는 것 중에 뒤가 낫다.

#### 35.2.1 배지·동작 열은 검사기가 본다

위 표는 **문서에만 있었고, 그래서 새어 나갔다.** `align` 을 빼먹어도 **타입은 통과하고
화면도 안 깨진다** — 배지만 열 왼쪽에 치우쳐 설 뿐이다. 회원 목록 두 열을 고친 뒤
**나머지 29곳이 그대로 남아 있었다**(`scripts/check-table-align.ts` 로 세었다).

검사기는 두 가지만 본다.

| 대상 | 판정 |
|---|---|
| `render` 가 **배지 하나만** 돌려주는 열 | `align: 'center'` 필수 |
| `labelHidden` 을 쓴 **동작 열** | `align: 'center'` 필수 |

⚠️ **배지가 섞여 든 열은 대상이 아니다.** 썸네일 + 이름 + 배지처럼 여러 조각을 그리는 열은
이름이 기준이라 좌측이 맞다. **검사가 멀쩡한 열을 잡기 시작하면 사람이 검사를 끈다**(§39.2).
...
클라이언트에 그 연출이 없다. 그래서 `mocks/nests.ts` 에 **줄을 더하는 `upsert` 가 없다** —
빠뜨린 게 아니다.

⚠️ **읽기 전용은 아니다.** 그림 교체와 해금 일수 수정은 연다(§19.3) — 줄이 고정인 것과
내용이 고정인 것은 다른 말이다.

한동안 `domain/nest.ts` 는 폴더가 아니라 **파일**이었다. 규칙이 하나도 없어서 `rules.ts` 가
빈 파일이 된다는 이유였는데, 경계에서 표시를 파생하기로 하면서 규칙이 생겨 **폴더로
바꿨다**(§41.5). 기준이 개수가 아니라 내용이라는 것은 그대로다 — 내용이 바뀌면 판단도 바뀐다.
...
### 41.5 해금 조건은 두 칸에 있었다

원본은 같은 날짜를 **카드(`sub`)와 표(`cond`) 두 곳에** 문자열로 들고 있었다. 경계를 고치면
둘 다 손대야 하고, 하나를 빠뜨리면 **같은 화면이 서로 다른 일수를 말한다.**

지금은 `fromDay` 하나만 저장하고 `unlockLabel` 이 만든다(§19.3.1). 카드와 표가 **같은 함수의
결과를 받아** 그리므로 어긋날 수 있는 경로가 없다.

### 41.6 ⚠️ 둥지 미리보기는 배경을 깔아야 읽힌다

둥지는 배경 위에 얹히는 오브젝트라 좌표계가 배경과 같고(`0 0 586 576`) **실제 형상이 아래쪽
1/4 에만 있다**(§8.6 에서 크롭하지 않기로 한 결과다).

그래서 **파일만 띄우면 위쪽 3/4 이 비어 보인다** — 멀쩡한 그림인데 올린 사람은
「잘못 올렸나?」 로 읽는다. 규격 검사가 가로세로를 보지 않기로 하면서 「그림이 맞는지는
미리보기로 사람이 본다」 고 정해 뒀는데(§8.5), 그 미리보기가 제 몫을 하려면 **합성이어야
한다.** `CompositePreview` 가 배경 위에 얹어 보여 준다.
...
두 공간 사이의 변환은 **클라이언트가 들고 있고 이 저장소에는 없다.** 겹쳐 그리면 우리가
지어낸 자리를 보여 주게 된다 — 확인하려던 것이 「자리에 맞는가」 인데 **틀린 자리를 보여
주면 없느니만 못하다.** 캐릭터는 리그 프레임 위에서 앵커선(중심축 · 접지선)과 맞춰 본다.

⚠️ **미리보기 틀은 폭이 아니라 높이로 잡는다.** 창 폭을 다 쓰면 캐릭터 프레임이 806px 까지
자라 **확인·취소 버튼이 화면 밖으로 밀린다.**

⚠️ **고른 것이 빌드 에셋이면 `assetId` 로 찾아야 한다.** `src` 만 보면 **올린 그림만** 뜨고
「있는 것 중에서 고르기」 는 빈 칸이 된다 — 실제로 그렇게 비어 있었다.

---

## 42. 성장 단계 (`/characters/growth`)

원본 `riruti-admin-char.dc.html` 의 마지막 화면. 카드 4 + 표(단계 · 소요 · 해금)뿐이라
가장 작지만, **아트가 없는 채로 붙인 유일한 화면**이라 그 처리 방식이 요점이다.

### 42.1 그림 없이 붙였다

`as_growth_0..3` 을 **id 로만 참조**한다. 디자이너가 원본을 `design/` 에 넣고
`bun run assets` 를 돌리면 **코드를 고치지 않아도 붙는다** — 그때까지는 `AssetThumb` 이
`?` 로 그린다.

⚠️ **「아트 준비 중」 같은 별도 표시를 만들지 않았다.** 그러면 그림이 온 뒤에 그 표시를
지우는 일이 남고, 지우는 것을 잊으면 **그림이 있는데 준비 중이라고 말하는 화면**이 된다.
`?` 는 이미 「이 자리에 그림이 있어야 하는데 없다」 를 말하고, 그림이 오면 저절로 사라진다
(유료 배경 4종과 같은 처리, §41.4).

### 42.2 왜 뽑지 못했나

§8.6 이 배경·둥지·업적을 원화에서 뽑았지만 성장 단계는 못 뽑았다. 이유는 개수가 아니라
**좌표계와 성질**이다.

| 단계 | 원화에 있나 | 왜 못 쓰나 |
|---|---|---|
| 알 | `riruti-art-6.dc.html` 에 인라인 SVG | **좌표계가 다르다.** 그 그림은 `0 0 120 140` 짜리 별도 삽화이고, 어드민은 `as_growth_*` 를 **의상 좌표계**(`298 -6 341 491`)로 기대한다. 맞추려면 크기를 지어내야 한다 |
| 금 | 없다 | 「금」 은 어드민만의 개념이다 — 아트는 알·유체·성체 3단계뿐이다 |
| 유체 · 성체 | `루티새v2` 프리셋 | **새 전체**(몸·볏·부리·눈·발·표정)를 그려야 해서 `sc-if` 120개짜리 템플릿을 통째로 평가해야 한다. 배경·둥지처럼 자기 완결적 구간을 잘라내는 일이 아니다 |

### 42.3 파사드를 뒤늦게 만든 이유

원래는 화면 상수뿐이었고 그 판단이 여기 적혀 있었다 — 「**측정값이 하나도 없이 정의뿐**이라
서버가 내려줄 것이 없다」. 그때는 맞았다. 같은 「고정 집합」 이라도 둥지에 파사드를 둔 것은
보유율(92 / 54 / 18%)이라는 측정값이 있어서였고, 기준은 개수가 아니라 **서버가 알아야 하는
값이 섞여 있는가** 였다.

**그림과 경계를 어드민에서 고치기로 하면서 그 전제가 깨졌다**(§19.3). 내려줄 것도 저장할
것도 생겨서 `domain/growth` · `mocks/growth.ts` · `api/growth.ts` 를 신설했다.

⚠️ **줄은 여전히 고정이다** — 파사드에 `create` 도 `remove` 도 없는 것은 빠뜨린 게 아니다.

### 42.4 ⚠️ 「금」 은 날짜 축에 없다

소요가 `'부화 직전'` 인 것은 오타가 아니다. **시간이 아니라 사건(부화)으로 끝나는 단계**라
날짜가 없다 — 표에서 유일하게 기간이 아닌 칸이라 고치고 싶어진다. 네 단계 중 **셋만 경계를
갖는다.**

타입으로 갈라 두었다. 하나로 합치면 「금」 에 일수를 넣는 코드가 짜여 버린다.

```ts
type GrowthStage =
  | { kind: 'days'; fromDay: number; … }   // 알 · 유체 · 성체
  | { kind: 'event'; note: string; … }     // 금
```

⚠️ **다음 단계를 찾을 때 사건 단계를 건너뛴다.** 「금」 이 알과 유체 **사이에** 놓이므로,
바로 다음 칸을 보면 일수가 없어 알의 표시를 만들 수 없다.

### 42.5 이로써 화면 52개가 전부 붙었다

`domain/screens.ts` 의 52개가 모두 `app/router.tsx` 의 `IMPLEMENTED` 에 있다.
`PlaceholderPage` 를 타는 화면은 더 이상 없다.

---

## 43. 로딩 표시 — 글자가 아니라 자리

`shared/ui/Skeleton.tsx`. 「불러오는 중…」 텍스트보다 나은 이유는 글자가 없어서가 아니라
**데이터가 도착할 때 화면이 튀지 않기** 때문이다. 거기서 원칙 셋이 따라온다.

1. **모양이 같아야 한다.** 카드가 올 자리엔 카드, 표가 올 자리엔 행.
2. **이미 아는 것은 지우지 않는다.** 제목·부제·버튼은 데이터가 없어도 그릴 수 있다.
3. **모르는 것은 흉내 내지 않는다.** 차트 안을 그리면 실제와 다른 그림이 되어 오히려 튄다.

### 43.1 모양이 하나뿐이면 목적을 배신한다

바꾸기 전에는 `Skeleton` 하나(목록 행)를 **44개 화면이 전부** 썼다. `rows={8}` 23곳,
`rows={6}` 16곳. 그런데 실제 화면은 카드 격자 33 · 표 27 · 지표 타일 21 · 폼 9 다.

**배경 20장이 들어올 자리에 목록 행 8줄을 그리고 있었다.** 자리를 잡아 주기는커녕 더 크게
튄다. 그래서 이름을 `SkeletonRows` 로 바꿨다 — `Skeleton` 이라는 이름이 「아무 데나 이걸
쓰면 된다」 고 말하고 있었고, 그게 이 상태를 만들었다.

| | 언제 |
|---|---|
| `SkeletonRows` | 목록·표 |
| `SkeletonCards` | 카드 격자 |
| `SkeletonStats` | 지표 타일 줄 |
| `SkeletonForm` | 폼(좌 입력 + 우 사이드) |
| `SkeletonBlock` | 차트처럼 안을 흉내 낼 수 없는 것 |
| `SkeletonPage` | **라우트 폴백 전용** (§43.3) |

⚠️ **`min` 은 그 화면의 `minmax` 와 같은 값이어야 한다.** 배경 190 · 업적 200 · 둥지 280 ·
성장 170 · 지표 200 으로 **전부 다르다.** 실제로 `SkeletonStats` 를 150 으로 고정해 두었더니
지표 6개가 **5 + 1 로 줄바꿈**되어 데이터가 올 때 한 줄로 다시 접혔다. 상수로 뽑지 말고
화면이 자기 값을 넘긴다.

⚠️ **높이도 맞춰야 한다.** `SkeletonStats` 를 두 줄로 두었더니 실제 `StatCard`(라벨·숫자·증감
**세 줄**)보다 짧아 아래가 통째로 밀렸다. 열 수만 맞추면 되는 게 아니다.

**맞았는지는 재서 확인한다.** 반대로 `min` 을 190 → 300 으로 되돌려 넣으면 배경 목록이
**4열 344px 대 7열 191px** 로 벌어진다 — 규칙이 실제로 일하고 있다는 것을 이렇게 확인했다.
...
| 둥지 카드 | 522 | 521 |
| 아이템 타일 | 225 | 223 |
| 종류 타일 (`lines={3}`) | 273 | 278 |
| 업적 카드 (`thumb={74}`) | 185 | 185 |
| `StatTile` | 67 | 67 |
| `StatCard` (`variant="card"`) | 109 | 110 |

### 43.1.1 ⚠️ 막대 높이는 글줄 높이가 아니다

**같은 개수의 막대를 쌓아도 카드가 짧아진다.** 스켈레톤 막대는 글자보다 얇게 그리는 게
보통인데(10px 막대 vs 19px 글줄), 그 차이가 줄마다 쌓여 카드 하나에서 10~28px, 두 행이면
50px 이 된다. 처음에 배경 239 대 249 · 업적 168 대 185 · `StatCard` 93 대 110 이 났다.

**여백은 실제 카드와 같은 값으로 두고, 모자란 만큼을 막대 높이와 줄 간격으로 되돌린다.**
여백을 부풀리면 「실제 카드와 같은 값」이라는 대조점이 사라져서, 나중에 실제가 바뀔 때
무엇을 따라가야 하는지 알 수 없게 된다.

**줄 수도 맞춰야 한다.** 배경·둥지·아이템은 타일 아래가 두 줄인데 **캐릭터 종류는 셋**
(이름 · 코드 · 배지 줄)이라 `lines={3}` 이 필요했다. **업적은 아예 다른 모양**이다 —
칸을 채우는 타일이 아니라 74px 그림을 가운데 놓고 구분선 아래 달성률이 붙어서 `thumb={74}`
로 갈랐다.

**모양이 카드 격자가 아닌 화면도 있었다.** 관리자 초대(273 → 861)와 상점 진열(190 → 516)은
카드 격자가 아니라 **2단 레이아웃**이다. `SkeletonBlock` 으로 좌우 열의 자리만 잡는다 —
안을 흉내 낼 수 없을 때 쓰는 것이 그것이다(원칙 ③). AI 심사도 지표 격자를 한 줄로 펴고
있었는데, 실제는 **지표 2 : 요약 카드 1** 의 2단이다.

**목록 행이 답이 아닌 화면도 있다.** `/ops/events` 는 표가 아니라 **152px 짜리 카드가 세로로
쌓이는** 화면이라, 45px 행 6줄로는 자리의 1/3 밖에 못 잡는다. 안을 흉내 낼 수 없으니
`SkeletonBlock height={152}` 세 장으로 **높이만** 잡았다(원칙 ③).

**보기 전환이 있는 화면은 `view` 를 따라간다.** 아이템 목록·캐릭터 종류는 격자와 표를 오가는데,
**`view` 는 데이터가 아니라 URL 이 정하므로 로딩 중에도 어느 모양이 올지 안다.**

### 43.2 헤더는 로딩 중에도 그린다

**규칙**: 화면이 헤더를 먼저 그리고 **본문만** 스켈레톤으로 채운다. `return <Skeleton />` 로
화면을 통째로 갈아 끼우던 **28개를 전부 고쳤고, 지금 남은 곳은 없다.**

화면 종류마다 「무엇이 고정이고 무엇이 데이터인가」가 달라서 셋으로 갈렸다.

| | 로딩 중에 그리는 것 | 못 그리는 것 |
|---|---|---|
| **폼 8** | 제목·부제 (`const FORM_SUB`) | **버튼** — 아직 없는 데이터를 대상으로 하는 동작이다 |
| **상세 10** | 부제·버튼 | **제목** — 「소이」 처럼 불러온 값이다 (`SkeletonHeader`) |
| **목록 10** | 헤더 전부. 가드를 JSX 안으로 옮겼다 | — |

⚠️ **제목이 불러온 값이면 그건 못 그린다.** 그 자리는 막대(`SkeletonHeader`)로 두고, 부제·
버튼처럼 **고정인 것만** 그린다. 아는 것과 모르는 것을 섞지 않는다.

⚠️ **폼의 버튼은 헤더에 있어도 뺀다.** 「저장」 은 로딩 중에 눌러 봐야 저장할 값이 없다.
헤더를 유지하는 목적은 자리를 잡는 것이지 **동작을 앞당기는 것이 아니다.**

### 43.2.1 지표 줄도 「아는 것」이 아니다 — 자리는 잡는다

목록 화면 8곳이 지표 타일을 `{data && (…)}` 로 감싸고 있었다. 로딩 중에는 **줄이 통째로
없다가** 데이터가 오면 나타나서, 그 아래 필터·표가 한 번에 밀렸다.

```tsx
{data ? (…타일…) : isPending ? <SkeletonStats count={4} min={150} silent … /> : null}
```

⚠️ **`isPending` 을 한 번 더 본다.** `{data ? … : <SkeletonStats/>}` 로 두면 **조회가 실패했을
때도** `data` 가 없어서 스켈레톤이 영원히 맥동한다 — 오류 배너 위에서 무언가 오는 중인 것처럼
보인다. 「없다」와 「오는 중」은 다른 상태다.

⚠️ **`count` 는 그 화면의 타일 수와 같아야 한다.** 공지·젬·FAQ·코드 상세는 타일이 **3개**인데
4를 그리고 있었다 — `auto-fit … 1fr` 이라 개수가 곧 열 수이고, 4열로 그렸다가 3열로 다시
짜인다. `min` 만 맞추면 되는 게 아니다.

⚠️ **`variant` 를 빼면 26px 이 밀린다.** `StatTile`(두 줄 · `13px`)과 `StatCard`(세 줄 ·
`15/16px`)는 실측 67px 대 110px 로 다르다. **기본은 `tile`** 이다 — 소비자가 19 대 1 이고,
`card` 는 지표 화면뿐이다. 새 화면이 `StatTile` 을 쓰면 기본값이 이미 맞다.

### 43.3 라우트 폴백은 어떤 화면이 올지 모른다

`AdminLayout` 의 `<Suspense fallback>` 은 lazy 청크를 기다리는 시점이라 화면 컴포넌트가 아직
없다. 그래서 `SkeletonPage` 는 **공통 골격**(제목 자리 + 본문 블록)만 그린다.

`matchScreen(pathname)` 으로 화면 id 는 알 수 있지만 **그 화면이 어떤 모양인지는
`domain/screens.ts` 가 모른다.** 넣으려면 도메인에 표현 정보를 들이게 되고(§4.4), 얻는 것은
청크가 도착하기까지 수백 ms 동안의 정확도뿐이다. 정확한 모양은 **데이터 로딩 스켈레톤**이
맡는다 — 그때는 화면이 자기 모양을 안다.

### 43.4 접근성 — 자동 검사가 못 보는 자리

⚠️ **막대는 읽히면 안 되고, 상태는 한 번만 말한다.** 빈 `div` 수십 개를 읽어 주는 것은
소음이라 안쪽은 전부 `aria-hidden` 이고 껍데기만 `role="status"` 다. 한 화면에 여러 개를 겹쳐
쓸 때는 하나만 남기고 `silent` 를 준다 — 안 그러면 「불러오는 중」을 두 번 읽는다.

⚠️ **`prefers-reduced-motion` 을 존중한다.** 스켈레톤이 화면을 통째로 채우게 되면서, 전정기관이
예민한 사람에게 **넓은 면적이 맥동하는 것**은 작은 막대 몇 개와 전혀 다른 경험이 됐다.
`rvPulse` 도 면적이 커진 만큼 얕게(0.55 → 0.72) 느리게(1.4s → 1.8s) 낮췄다.

**둘 다 Lighthouse 가 보지 않는다.** 「빈 div 가 상태를 알리는가」도 「넓은 면적이 움직이는가」도
자동 검사 대상이 아니라, 여기 적어 두는 것이 유일한 방어다.

### 43.5 버튼 안의 「…중」은 로더가 아니다
...
#### 44.3.1 ⚠️ **창 안의 Enter 도 바깥 폼을 제출한다** — `blocked` 로는 못 막는다

위 가드는 **폼이 아직 덜 채워졌을 때**를 막는다. 다 채운 폼에서는 `blocked` 가 거짓이라
그대로 통과한다 — 그런데 **모달이 떠 있어도 통과한다.**

`<dialog>` 는 top layer 로 띄워 줄 뿐 **폼의 경계가 아니다.** 창이 `<form>` 안에
그려져 있으면 창 안의 `<input>` 도 그 폼의 소유라, 브라우저는 Enter 를 암묵적 제출로
처리한다. 바깥 버튼은 `inert` 로 잠겨 있지만 **눌린 버튼이 없어도 제출은 난다.**

```text
실측 — 챌린지 등록 폼에서 「보상 아이템 고르기」 를 열고 검색칸에 Enter
  고치기 전  /challenges/new → /challenges/18   보상 없는 챌린지가 만들어졌다
  고친 뒤    /challenges/new 그대로 · 창 1개 · 포커스 검색칸
```

**`Dialog` 가 한 번에 막는다.** 화면마다 `onKeyDown` 을 다는 대신 창 자신이 처리한다 —
중첩 창의 Esc 와 같은 자리다(§63.1). 조건은 **소유 폼이 창 밖일 때만**이다. 창이 자기
`<form>` 을 품고 있으면 거기서의 Enter 는 그 폼의 것이 맞다.

```ts
if (el.form && !ref.current?.contains(el.form)) e.preventDefault()
```

⚠️ **`textarea` 는 건드리지 않는다.** 여러 줄 입력에서 Enter 는 줄바꿈이라 애초에 제출로
새지 않는다. `Dialog.test.tsx` 가 넷을 고정한다 — 바깥 폼 · 자기 폼 · 폼 없음 · textarea.

### 44.4 오류는 **제출을 눌러 본 뒤**에 보여준다

폼이 두 규약으로 갈려 있었다 — 8곳은 `tried`(제출 뒤), 5곳은 `isDirty`/`touched`(건드리면
바로). `tried` 로 모았다.

**빈 폼을 열자마자, 한 칸만 채웠을 때 빨개지지 않는다.** `isDirty` 쪽은 이름만 쳐도 가격·
에셋·노출이 동시에 붉어졌다 — 아직 채우는 중인 사람에게 「틀렸다」고 말하는 것이다.
무엇이 남았는지는 **우측 체크리스트**가 계속 말하므로 정보가 사라지지도 않는다.

⚠️ **`touched` 를 그대로 쓰면 안 된다.** 챌린지·종류의 `touched` 는 **미저장 경고와 초안
자동 저장**을 겸하고, 되살린 초안이면 **참으로 시작한다.** 그걸로 오류를 그리면 초안을
되살리자마자 빨개진다 — 되돌려 넣어 확인했다(복원 직후 `aria-invalid` 3개). 표시 게이트는
`tried` 로 따로 센다.

**제출 버튼은 잠그지 않는다.** `blocked` 로 잠그던 5곳을 풀었다 — 잠긴 버튼은 **왜 잠겼는지
말하지 못한다**(§22.2.3). 누르면 무엇이 왜 남았는지 그 자리에서 보인다.

> 예외 하나: 종류 등록의 `codesUnknown`(중복 검사에 쓸 목록이 아직 안 옴)은 **오류 목록에
> 없어서** 눌러도 설명할 것이 없다. 거기는 잠그되 `title` 로 이유를 말한다(§18.8).

### 44.5 ⚠️ 첫 오류로 포커스를 옮긴다 — 두 번 틀렸다

네이티브 검증이 해 주던 나머지 하나가 이것이다. 없으면 긴 폼에서 오류 칸이 화면 밖일 때
**누른 것이 아무 일도 안 일어난 것처럼 보인다.** `shared/lib/focusFirstError.ts` 가
`[aria-invalid="true"]` 첫 번째를 찾아 `focus()` 한다 — `Input`·`Textarea`·`Select`·
`FilePicker` 넷 다 **포커스를 받을 수 있는 요소**에 그 속성을 붙인다.

만드는 과정에서 두 번 틀렸고, 둘 다 **눈으로는 안 보이는** 종류였다.

**① `requestAnimationFrame` 으로 미뤘다 — 탭이 화면에 없으면 아예 안 돈다.**
`visibilityState: 'hidden'` 에서 rAF 콜백이 발화하지 않는 것을 실측했다. 「보이지 않을 때만
조용히 안 되는」 동작이라 사람이 보고 있는 동안에는 절대 재현되지 않는다.

**② 그래서 `flushSync` 로 바꿨다.** 오류 표시는 `tried` 를 켠 **렌더에서야** DOM 에 붙는다.
그냥 `setTried(true)` 하면 다음 줄에서 아직 안 붙어 있어 옮길 대상을 못 찾는다.

```tsx
flushSync(() => setTried(true))   // 이 줄이 끝나면 aria-invalid 가 DOM 에 있다
if (blocked) {
  e.preventDefault()
...
⚠️ **`setTried` 를 `form.handleSubmit(...)` 안에 두지 말 것.** 저건 비동기라 상태 갱신이
밀리고, 제출 게이트가 두 곳(콜백 안·밖)으로 갈린다. 동기 제출 핸들러 하나가 게이트다.

⚠️ **`scrollIntoView` 를 쓰지 않는다** — 조상까지 스크롤해서 본문이 같이 튄다(§9.3의 탭
스트립과 같은 이유). `focus()` 는 필요한 만큼만 움직인다.

오류가 **입력 칸이 아닌 것**에만 있으면(에셋 미선택 같은) 옮길 곳이 없어 아무 일도 하지
않는다 — 그 경우에도 메시지와 체크리스트는 이미 그려져 있다.

### 44.5.1 ⚠️ 제출 게이트는 **진행 중**도 봐야 한다

버튼을 잠그는 것만으로는 못 막는다 — **한 줄 입력의 Enter 는 버튼을 거치지 않는다**(§44.3과
같은 구멍). 재현했다: 챌린지 등록을 세 번 제출하니 **셋 다 만들어졌다**(`/challenges/18`
대 `/challenges/20`).
...
⚠️ **`upload.isPending` 도 봐야 한다.** 아이템·업적·배경은 `handleSubmit` 안에서 업로드를
**기다린 뒤에야** `save.mutate` 를 시작한다 — 업로드가 도는 동안에는 `save.isPending` 이
아직 **false** 다. 아이템 폼은 버튼 조건에도 그게 빠져 있어서, 업로드 중에 다시 누르면
**업로드와 저장이 통째로 두 번** 나갔다.

⚠️ **상태만으로는 부족하다 — `useRef` 로 닫는다.** `busy` 는 React 가 「진행 중」을 커밋한
**뒤부터** 참이라, 같은 태스크에서 동기로 연달아 제출하면 두 번째가 아직 옛 값을 본다.
실측으로 **3회 중 2개**가 만들어졌다. `ref` 는 즉시 반영되므로 그 틈이 없다.
...
⚠️ **푸는 자리를 빠뜨리면 폼이 영영 잠긴다** — 막으려던 것보다 나쁜 고장이다. 끝나는 길마다
하나씩 있어야 한다: **`onSettled`**(성공·실패 **모두** 온다. `onSuccess` 에만 두면 한 번
실패한 폼은 다시 못 보낸다)와 **업로드 실패의 조기 반환**. 둘 다 목에 실패를 주입해
확인했다 — 저장 실패·업로드 실패 뒤에도 재제출이 매번 돈다(1 → 2 → 3회차).

중복의 **마지막 방어선은 화면이 아니라 파사드와 서버**라는 것은 그대로다(§22.2.3).

> 확인하다 **다른 것을 하나 찾았다.** `ItemFormPage` 는 주석에 「오류는 `upload.error` 로
> 화면에 나온다」 고 적어 두고 **정작 그리지 않고 있었다**(업적·배경은 그린다). 이미지
> 업로드가 실패하면 **아무 일도 안 일어난 것처럼** 보였다. 같이 고쳤다.
...
## 46. 이벤트 생성 — 기간·보상·강조색을 하나의 저장 계약으로 묶는다

### 46.1 목록이 소비하는 값까지만 만든다

`design/riruti-admin-ops.dc.html` 에는 이벤트 목록과 「이벤트 생성」 버튼만 있고 생성 폼
원본은 없다. 따라서 새 화면은 기존 카드가 실제로 소비하는 제목·설명·기간·보상 아이템·강조색을
입력하는 형태로 그린다. 디자인 원본에 없는 참여 조건이나 노출 대상을 임의로 발명하지 않는다.

이번 범위:

- `/ops/events/new` 생성 화면과 목록 버튼 연결
- 제목 · 설명 · 시작일 · 선택 종료일 · 보상 아이템 · 강조색 입력
- 보상 아이템의 이미지·이름 미리보기와 강조색 미리보기
- 작성 중 초안 복원과 미저장 이탈 경고
- 목 저장 후 이벤트 목록 갱신과 목록 복귀

이번에 하지 않는 것:

- 이벤트 상세 · 수정 · 삭제
- 참여 조건 · 대상자 · 보상 수량처럼 현재 조회 타입에 없는 정책
- 실제 서버 DTO와 HTTP 연결

### 46.2 화면은 기존 「이벤트」 탭 안에서 바뀐다

화면 id는 기존 규칙대로 소문자 `eventnew` 로 둔다.
...
`section: 'event'` 이므로 생성 화면은 새 탭을 만들지 않고 「이벤트」 탭 안에서 열린다. 저장
성공 뒤 `/ops/events` 로 돌아간다. 목록 버튼은 비활성 상태를 없애고 `useNavigate` 로 이 경로를
연다.

### 46.3 입력과 조회 타입을 분리한다

`OpsEvent` 의 `key` 와 `joined` 는 운영자가 정하지 않는다. 폼과 저장 파사드는 별도 입력
타입만 받는다.
...
새 이벤트의 `joined` 는 0이고 `key` 는 목 저장소가 발급한다. `rewardItemKey: null` 은 폼에서
아직 보상을 고르지 않은 상태이며 저장 가능한 이벤트에는 남지 않는다. 검증 뒤에도 단언으로
밀어 넣지 않고 값이 실제 아이템 목록에 있는지 저장 시점에 다시 확인한다.

### 46.4 기간·강조색·보상은 파사드에서도 다시 검증한다

`validateEvent` 가 화면 오류를 만들고 `saveEvent` 가 같은 검증을 다시 수행한다.
...
| 종료일 | 빈 값 또는 실제 날짜, 시작일 이상 |
| 강조색 | 정확한 `#RRGGBB` |
| 보상 아이템 | 선택 필수, 저장 시점에도 존재하는 아이템 |

종료일을 비우면 상시 이벤트다. 문자열 비교는 실제 날짜 검증을 통과한 뒤에만 한다. 강조색은
사용자 입력이라 디자인 토큰이 아니며 카드 장식에만 쓴다. 텍스트 색이나 상태 전달에는 쓰지 않아
임의 색의 대비가 접근성을 깨뜨리지 않게 한다.

보상 선택지는 feature가 `mocks/` 를 직접 읽지 않도록 `getEventFormData` 파사드에서 받는다.
각 선택지는 `key`·`name`·`assetId` 를 가져 `<Select>` 와 `AssetThumb` 미리보기가 같은 데이터를
쓴다. 폼을 연 뒤 아이템이 삭제될 수 있으므로 `saveEvent` 는 `allItems()` 로 존재 여부를 다시
확인하고, 없으면 저장을 거절한다.

### 46.5 폼은 세 덩어리로 읽힌다

한 화면 안에서 다음 순서로 배치한다.

1. 기본 정보 — 제목과 설명
2. 운영 기간 — 시작일·종료일과 강조색 입력·색 띠 미리보기
3. 보상 — 아이템 선택과 썸네일·이름 미리보기

강조색은 텍스트 `Input` 으로 받아 값을 정확히 고칠 수 있게 하고, 유효한 값일 때만 옆 미리보기에
적용한다. 네이티브 색상 선택기만 두면 정확한 서버 값 확인과 키보드 입력이 어려워진다. 보상
선택지를 불러오는 동안은 `PageHeader` 와 `SkeletonForm`, 실패하면 `ErrorBanner` 를 그린다.
아이템이 하나도 없으면 이유를 표시하고 제출을 막는다.

### 46.6 공지 작성에서 고정한 폼 안전장치를 그대로 쓴다

`EventFormPage` 는 §45.6의 순서를 따른다.

1. `useFormDraft('event:new', values, dirty)` 로 초안을 저장·복원한다.
2. `useUnsavedGuard` 로 새로고침과 탭 닫기의 손실을 막는다.
3. 제출을 시도한 뒤에만 필드 오류를 표시한다.
4. `flushSync` 뒤 `focusFirstError` 로 첫 오류에 포커스한다.
5. `useRef` 동기 잠금과 mutation 상태로 연속 제출을 막는다.
6. 성공하면 초안과 더러움 표시를 지우고 목록으로 돌아간다.
7. 성공·실패 양쪽을 받는 `onSettled` 에서 잠금을 풀어 실패 뒤 재시도를 보장한다.

초안의 보상 key가 다시 열었을 때 선택지에 없으면 선택하지 않은 상태로 정규화한다. 사라진
아이템 이름을 그럴듯하게 남겨 잘못된 보상을 저장시키지 않는다.

### 46.7 목 저장소와 query 경계

현재 초기 이벤트는 오늘을 기준으로 날짜를 계산한다. 이 동작을 보존하기 위해 초기 튜플은
얼리지 않고 새 이벤트만 `ADDED_EVENTS` 에 쌓는다. `allEvents()` 는 새 이벤트를 앞에 붙인 뒤
초기 이벤트를 만든다.

`addEvent(input)` 은 key를 발급하고 `joined: 0` 으로 저장한다. 테스트 격리를 위해
`resetEvents()` 를 제공한다. `useSaveEvent` 성공 시 `qk.ops.events()` 를 invalidate해 저장된
이벤트가 목록에 다시 나타나게 한다. 선택지 query는 아이템 목록과 수명이 다르므로 별도
`qk.ops.eventForm()` 키를 쓴다.

실서버 연결 시 화면 계약은 유지하고 `getEventFormData` 와 `saveEvent` 안을 각각 조회·생성
HTTP 호출로 교체한다. 서버도 날짜·강조색·보상 존재 여부를 저장 시점에 다시 검증해야 한다.

### 46.8 검증

- `validateEvent`: 빈 값, 존재하지 않는 날짜, 종료일 역전, 상시 이벤트, 강조색 형식, 보상 누락
- `addEvent`: key 발급, joined 0, 앞쪽 삽입, 입력 문자열 정리
- `saveEvent`: 잘못된 입력과 사라진 보상 거부, 성공 뒤 query 무효화
- 브라우저: 빈 제출의 첫 오류 포커스, 초안 복원, 사라진 보상 정규화, 강조색·보상 미리보기,
  동기 5연속 제출 1건, 실패 뒤 재시도, 저장 뒤 목록 반영
- 화면을 추가하므로 `bun run a11y` 포함 전체 검증
- 핵심 검증 하나를 의도적으로 되돌려 관련 테스트가 실제로 실패하는지 확인한 뒤 복구

### 46.9 이제 등록 화면은 54개다

`eventnew` 가 더해져 `domain/screens.ts` 와 `app/router.tsx` 는 각각 54개다. 모두
`IMPLEMENTED` 에 등록해 새 라우트가 `PlaceholderPage` 로 떨어지지 않게 한다.

---

## 47. 푸시 재발송 — 기록을 수정하지 않고 새 작성 화면을 연다

### 47.1 재발송은 복제가 아니라 새 발송이다

발송 완료 상세의 「같은 내용으로 재발송」을 `/ops/push/new?from=:pushId` 로 잇는다. 과거
기록을 다시 실행하거나 수정하지 않고, 원본을 읽어 새 `PushInput` 의 초기값으로만 쓴다.
따라서 새 발송은 새 key·발송자·발송 시각·통계를 갖고 기존 상세는 그대로 남는다.

복사하는 값은 종류·제목·본문·이동·대상 세그먼트다. 복사하지 않는 값은 다음과 같다.

- `now` 는 `true`, `at` 은 빈 값 — 과거 시각으로 다시 예약할 수 없다.
- 도달·열림·발송자·상태 — 서버가 새 발송 결과로 정한다.
- 직접 지정 회원 ID — 현재 `Push` 기록은 대상 수만 보존하고 ID 목록은 갖지 않는다.

직접 지정 발송은 `audience: '직접 지정'` 을 유지하되 `ids` 를 비운다. 화면은 필수 오류로
대상을 다시 입력하게 한다. 모르는 과거 대상 대신 「전체」를 넣으면 범위가 조용히 넓어져 더
위험하다.

### 47.2 원본 id는 URL에 남긴다

라우트 state만 쓰면 새로고침하는 순간 복사 원본이 사라진다. `from` query에 key를 남기고
`usePush(from)` 으로 다시 읽는다. 값이 없으면 평소의 빈 작성 화면이고, 값이 있는데 원본을
찾지 못하면 빈 폼으로 조용히 바꾸지 않고 오류를 보여 준다.

상세 버튼은 발송 완료 건에만 보이는 기존 조건을 유지한다. 예약 건은 「예약 취소」 흐름이고,
취소 건은 같은 내용을 다시 보내려면 목록에서 발송 완료 원본을 골라야 한다.

### 47.3 초안은 원본별로 나눈다

일반 작성은 `push:new`, 재발송은 `push:resend:<pushId>` 초안 칸을 쓴다. 둘이 같은 칸을 쓰면
새 알림 초안이 재발송 원본을 덮거나, 반대로 재발송 화면에서 무관한 새 알림 초안이 복원된다.

원본 조회가 끝나면 복사값을 폼의 초기 비교값으로 잡는다. 재발송 초안이 있으면 그 초안을 복사
원본보다 우선하며 「작성 중인 내용 복원」 알림을 그대로 보여 준다. 원본 로딩 중에는 입력 폼
대신 스켈레톤을 보여, 빈 폼을 먼저 편집했다가 조회 결과로 덮이는 상황을 막는다.

`pushDraftScope(sourceId)` 는 초안 칸이면서 편집기 React `key` 다. 같은 작성 라우트에서 query만
`A → B → 없음`으로 바뀌어도 편집기를 다시 만들어야 한다. 그렇지 않으면 A의 로컬 state가 남은
채 자동 저장 scope만 B로 바뀌어, A의 내용을 B 초안으로 쓰는 데이터 오염이 생긴다.

### 47.4 기존 안전장치를 그대로 통과한다

복사한 값도 `validatePush`·야간 마케팅 차단·직접 지정 대상 확인·발송 확인 Dialog·
`sendPush` 재검증을 그대로 거친다. 재발송 버튼이 API를 직접 부르면 이 안전장치를 전부
우회하므로 금지한다.

### 47.5 검증

- query: `from`이 없거나 빈 값이면 일반 작성, 있으면 원본별 초안 scope
- 변환: 복사 필드, 즉시 발송 초기화, 직접 지정 IDs 제거
- 브라우저: 발송 완료 상세 → 작성 화면, 복사값 표시, 직접 지정 대상 재입력, 새로고침 유지
- 라이트·다크와 `/ops/push/:id`·`/ops/push/new?from=:id` 접근성 100
- 변환에서 과거 `at`을 복사하도록 되돌렸을 때 테스트가 실패하는지 확인

---

## 48. 아이템 노출 전환 — 기록은 남기고 앱에서만 내린다

### 48.1 삭제나 수정 폼의 저장이 아니다

상세의 「미노출로 전환」은 아이템을 지우지 않고 `status`만 `HIDDEN`으로 바꾼다. 에셋·판매량·
보유율·지급 이력과 노출 기간은 그대로 남긴다. 운영자가 장애나 정책 문제로 판매를 급히 멈추는
동작이라 수정 폼을 열어 다시 저장하게 하지 않고, 영향 범위를 설명하는 확인 Dialog 뒤 즉시
반영한다.

미노출 아이템에서는 같은 자리가 「노출로 전환」이 된다. 다시 올릴 때 저장된 `visibleFrom`이
있으면 `SCHEDULED`, 없으면 `VISIBLE`로 돌아간다. 사람이 내린 미노출 결정을 기간 수정만으로
풀지 않는 `statusOf` 규칙과, 사람이 명시적으로 다시 올리는 이 동작은 서로 다르다.

### 48.2 데이터 경계

`visibilityStatusOf(item, hidden)`이 다음 상태를 정하고, 목 저장소는 그 상태만 반영한다.
`setItemVisibility`는 실서버 연결 시 `PATCH /admin/items/{id}/visibility`로 교체하며 성공하면
`qk.items.all`을 무효화한다. 따라서 현재 상세 배지와 목록 상태가 같은 저장 결과를 다시 읽는다.

### 48.3 검증

- 도메인: 내리면 항상 `HIDDEN`, 다시 올릴 때 시작일 유무에 따라 `SCHEDULED`/`VISIBLE`
- 브라우저: 확인 전에는 유지, 확인 뒤 배지·버튼 변경, 다시 올리면 원래 기간 규칙으로 복구
- 라이트·다크와 아이템 상세 접근성 100
- 복구 상태를 무조건 `VISIBLE`로 바꿨을 때 예약 복구 테스트가 실패하는지 확인

---

## 49. 회원 상세 → 문의 내역 — 없는 발신 기능을 만들지 않는다

### 49.1 「문의 남기기」가 아니라 「문의 내역」이다

회원 상세 원본에는 「문의 남기기」가 있지만 현재 1:1 문의는 **회원이 시작하고 운영자가
답변하는** 모델이다. 운영자가 새 대화를 보내는 API·상태·알림 계약은 없다. 버튼만 활성화해
운영자 말로 시작하는 `Inquiry`를 만들면 대기 시간과 답변 상태가 모두 거짓이 된다.

따라서 버튼을 「문의 내역」으로 바로잡고 `/support/inquiries?who=:uid`로 연결한다. 새 기능을
지어내지 않으면서 회원 상세에서 실제 후속 CS 화면으로 갈 수 있다. UID는 새로고침과 공유가
가능하도록 URL에 남긴다. `cs` 스코프가 없으면 버튼은 「문의 내역 · 권한 없음」으로 잠근다.
권한 밖 화면으로 보냈다가 다른 첫 화면으로 튕기게 하는 것은 연결이 아니다.

### 49.2 회원 필터는 화면에 보여야 한다

`who`는 API 파사드에서 `User.key`로 풀고 도메인의 `filterInquiries`에는 `userKey`를 넘긴다.
`Inquiry`가 회원을 key로 가리킨다는 기존 계약을 바꾸지 않는다. UID를 찾지 못하면 전체 문의로
범위를 넓히지 않고 0건을 반환하며, 배너도 raw UID를 회원 이름처럼 쓰지 않고 「회원을 찾을 수
없습니다」라고 밝힌다.

회원 필터는 검색창 아래에 이름·UID와 「전체 문의 보기」로 드러낸다. 주소에만 조건이 있고 화면에
표시가 없으면 운영자는 문의가 적은 이유를 데이터 누락으로 오해한다. 상태·분류·검색 필터를
바꿔도 `who`는 유지하며, 해제 버튼을 눌렀을 때만 제거한다. 상단 SLA 지표는 기존 규약대로
필터 전 전체 문의를 나타낸다.

### 49.3 검증

- query: `who`와 상태·분류·검색 조건을 함께 복원하고 모르는 열거값은 버린다
- 도메인: `userKey` 조건이 다른 회원 문의를 제외한다
- 브라우저: 회원 상세 → 해당 UID 문의만 표시 → 다른 필터와 병용 → 전체 문의 복귀
- 잘못된 UID가 전체 문의를 보이지 않는지 확인
- 라이트·다크와 회원 상세·문의 목록 접근성 100

---

## 50. 다른 관리자의 2단계 인증 초기화 — 복구 수단이 잠금 해제가 되지 않게 한다

### 50.1 자기 자신이 아니라 다른 최고 관리자가 복구한다

인증 앱을 잃은 관리자는 백업 코드도 없으면 로그인할 수 없다. 관리자 상세의 「2단계 인증」
카드에서 등록된 앱 OTP를 초기화해 다시 등록할 수 있게 한다. 다만 현재 로그인한 자기 계정은
초기화할 수 없다. 열린 세션 하나만으로 자기 2차 인증을 없앨 수 있으면 계정을 탈취한 사람이
복구 기능을 잠금 해제로 쓸 수 있기 때문이다. 다른 최고 관리자가 대상 계정을 열어 처리한다.

이미 `미설정`인 계정도 초기화할 것이 없으므로 버튼을 잠그고 이유를 함께 표시한다. 계정 정지
여부는 초기화를 막지 않는다. 분실 대응과 계정 접근 차단은 서로 다른 조치라, 정지 상태에서
인증 수단을 먼저 정리한 뒤 계정을 다시 열 수 있어야 한다.

### 50.2 확인 뒤 등록 상태만 지운다

확인 대화상자는 기존 인증 앱과 백업 코드가 모두 무효가 된다는 영향을 먼저 말한다. 성공하면
`Admin.mfa`만 `미설정`으로 바꾸고 역할·담당 모듈·계정 상태·활동 기록은 그대로 둔다. 화면은
관리자 상세 query를 다시 읽어 「미설정」과 잠긴 버튼을 즉시 보여 준다.

화면의 차단은 안내일 뿐이고 파사드가 같은 규칙을 다시 검사한다. 없는 관리자, 자기 계정,
이미 미설정인 계정은 각각 실패한다. 실서버 연결 시 이 경계는 현재 세션의 최고 관리자 권한을
검증하고, 등록된 TOTP 시크릿과 백업 코드 해시를 한 트랜잭션으로 폐기해야 한다.

### 50.3 검증

- 도메인: 자기 계정과 미설정 계정은 차단, 다른 등록 계정과 정지 계정은 허용
- 목 저장소: MFA 외의 계정 필드는 보존
- 브라우저: 확인 취소 시 유지, 확인 후 `미설정` 반영, 재실행 차단
- 라이트·다크 관리자 상세 접근성 100
- 자기 계정 차단을 제거했을 때 관련 테스트가 실제로 실패하는지 확인

---

## 51. 관리자 비밀번호 초기화 — 비밀번호를 만들지 않고 복구 링크만 보낸다

### 51.1 화면은 비밀번호를 알 수도 정할 수도 없다

관리자 상세의 「비밀번호 초기화」는 대상의 사내 이메일로 일회용 초기화 링크를 보낸다.
운영자가 임시 비밀번호를 만들거나 화면에서 확인하는 흐름은 두지 않는다. 전달 과정에서 비밀번호가
노출되고, 받은 사람이 바꾸기 전까지 발급자가 계정에 들어갈 수 있기 때문이다.

메일 발송만으로 현재 비밀번호와 열린 세션을 끊지 않는다. 링크에서 새 비밀번호 저장을 마친
시점에 서버가 기존 비밀번호와 다른 초기화 링크를 무효화한다. 메일이 늦거나 유실됐다는 이유로
정상 사용 중인 계정을 갑자기 잠그지 않기 위한 순서다.

### 51.2 초대 대기 계정은 기존 초대 흐름을 쓴다

`firstLoginAt`이 빈 계정은 아직 비밀번호를 설정한 적이 없다. 이 계정에 「초기화」 메일을 보내면
초대 링크와 초기화 링크 중 무엇으로 시작해야 하는지 두 경로가 생긴다. 따라서 비밀번호 초기화는
막고 「아직 로그인하지 않은 계정은 초대 링크로 시작합니다」라고 이유를 표시한다.

정지 계정에는 보낼 수 있다. 계정 접근 차단과 자격 증명 복구는 별개라, 비밀번호를 먼저 정리한
뒤 정지를 풀 수 있어야 한다. 자기 계정도 등록된 사내 메일을 실제로 받을 수 있으므로 허용한다.

### 51.3 확인과 결과

확인 창은 수신 주소와 현재 비밀번호·세션이 즉시 바뀌지 않는다는 점을 함께 알린다. 성공하면
대상 이메일로 보냈다는 문장을 화면에 남기고 버튼을 「초기화 메일 다시 보내기」로 바꾼다.
연속 클릭은 `Dialog.confirmDisabled`와 동기 ref 잠금으로 막는다.

실서버에서는 현재 HttpOnly 세션의 최고 관리자 권한을 검증하고, 링크 토큰은 짧은 수명·일회용
해시로 저장하며 발송 횟수를 제한해야 한다. 클라이언트가 넘긴 행위자 정보는 신뢰하지 않는다.

### 51.4 검증

- 도메인: 최초 로그인 전 계정만 차단, 활성·휴면·정지·자기 계정은 허용
- 파사드: 없는 관리자와 초대 대기 계정을 거절하고 등록된 이메일만 대상으로 삼음
- 브라우저: 취소 시 미발송, 확정 후 수신 주소 표시, 동기 연속 확인 1회
- 라이트·다크 관리자 상세 접근성 100
- 최초 로그인 판정을 제거했을 때 관련 테스트가 실제로 실패하는지 확인

---

## 52. 로그인 필드 오류 — 「어느 칸이 틀렸는지」를 말하게 한다

§44 로 폼 13개의 메시지를 전부 우리 것으로 만들었는데 **로그인만 빠져 있었다.**
`ErrorBanner` 한 줄이 전부라, 아이디가 틀렸는지 비밀번호가 틀렸는지 화면이 말하지 못했다.

### 52.1 문자열 하나로는 칸을 가리킬 수 없다

`validateCredentials` 는 `string | null` 을 돌려줬다. 그래서 **뭉친 문구**가 생겼다 —
「아이디와 비밀번호를 **모두** 입력해 주세요」. 둘 중 하나만 비어도 같은 말이 나온다.

다른 `validate*` 16개와 같은 모양으로 바꿨다.
...
⚠️ **`api/auth.ts` 의 호출은 남긴다.** 화면 검증은 편의고 파사드의 것은 **계약**이다 —
다른 호출부가 생겨도 형식이 깨진 값이 서버로 나가면 안 된다(§22.2.3). 배너에는 한 줄이
필요하니 `Object.values(errors)[0]` 로 첫 오류를 뽑는다(`saveNotice` 와 같은 방식).

### 52.2 `Field.tsx` 에 오류 배선이 아예 없었다

`shared/ui/Input.tsx` 가 이미 갖고 있던 계약을 그대로 옮겼다 — `error` → `aria-invalid` +
`aria-describedby` + 붉은 테두리 + 칸 아래 한 줄. `required` 도 함께 붙였다(전에는
`aria-required` 조차 없었다).

⚠️ **오류 줄의 `id` 는 `aria-describedby` 와 짝이어야 한다.** 아니면 화면에는 보이는데
스크린리더는 **왜 틀렸는지 못 듣는다.**

⚠️ **비밀번호 칸의 오류는 「보기」 토글을 감싼 `relative` 바깥에 둔다.** 안에 두면 버튼 위에
겹친다.

`Field.tsx` 를 `shared/ui` 로 올리지는 않았다 — 규약은 「두 번째 소비자에서 승격」이고
아직 로그인만 쓴다(§4.4).

### 52.3 ⚠️ `autoFocus` 때문에 포커스 검증이 거짓으로 통과한다

`flushSync` 를 빼서 회귀를 확인하려는데 **그래도 통과했다.** 아이디 칸에 `autoFocus` 가
있어서 **포커스가 처음부터 거기 있었고**, 프로그램적 `click()` 은 포커스를 옮기지 않기
때문이다 — `focusFirstError` 가 아무 일도 안 해도 결과가 같아 보인다.

**포커스를 다른 칸에 두고 재야 한다.**

```text
비밀번호 칸에 포커스를 둔 뒤 제출
  flushSync 없음 → 「비밀번호 *」 그대로   (focusFirstError 가 대상을 못 찾음)
  flushSync 있음 → 「아이디 *」 로 이동
```

### 52.4 검증

- 열자마자 `aria-invalid` 0 · 「로그인」 버튼 안 잠김
- 빈 폼 제출 → **두 칸 각각**에 메시지, 포커스는 아이디로, `aria-describedby` 가 그 메시지를 가리킴
- `not-an-email` → 「아이디는 회사 이메일 형식이어야 합니다.」가 **아이디 칸 아래**
- 고치면 오류가 사라지고, 목 계정으로 2차 인증 화면까지 간다
- `form.noValidate === true` — 네이티브 풍선은 여전히 막혀 있다(§44.2)
- 회귀 주입: `validateCredentials` 의 규칙 다섯을 하나씩 되돌려 넣어 전부 빨개지는 것을 확인

---

## 53. 두 가지 조용한 중복을 걷어낸다

기능이 아니라 **같은 규칙이 두 벌 있던 자리**를 하나로 모은 정리다.

### 53.1 ⚠️ 오류 배너가 방금 난 실패를 가리고 있었다

일곱 화면이 뮤테이션 둘의 오류를 이렇게 그렸다.

```tsx
{(a.error || b.error) && <ErrorBanner message={(a.error ?? b.error)!.message} />}
```

react-query 의 `error` 는 다음 `mutate` 나 `reset` 까지 **남아 있다.** 그래서 A 가 실패한
뒤 B 도 실패하면 **먼저 난 A 의 메시지만 계속 보인다** — 방금 누른 것이 왜 안 됐는지는
어디에도 안 나온다. 눌렀는데 아무 반응이 없는 것처럼 보인다.

FAQ 화면에 실패를 주입해 재현했다.

```text
① 「순서 저장」 실패        → 「순서 저장 실패」          ✓
② 이어서 노출 전환도 실패   → 여전히 「순서 저장 실패」만   ✗  (배너 1개)
   고친 뒤                 → 둘 다 보인다               ✓  (배너 2개)
```

배너를 **뮤테이션마다 하나씩** 그린다. `ErrorBanner` 가 자기 `mb` 를 갖고 있어 겹쳐 놓아도
간격이 맞는다.

| 고친 화면 |
|---|
| `TotpCard` · `ShopDisplayPage` · `PushFormPage` · `InquiryDetailPage` · `FaqFormPage` · `FaqPage` · `GrantsPage` |

> `AdminDetailPage` 는 이미 같은 방식으로 고쳐져 있었다(§50 작업 중에). 나머지를 맞춘 것이다.

### 53.2 같은 정규식이 이름만 다르게 둘이었다

캐릭터 종의 대표 색(`HEX`)과 이벤트 강조색(`isEventAccent`) 이 **글자까지 같은 규칙**을 각자
들고 있었다. 규칙이 하나인데 정의가 둘이면 한쪽만 고쳐지고 조용히 갈린다.

`shared/lib/color.ts` 의 `isHexColor` 하나로 모았다. **`domain/` 이 아닌 이유**는 「리루티를
몰라도 말이 되는가」 — `#RRGGBB` 인지 묻는 것은 도메인을 모른다(§4.4 의 `format.ts` 와 같은
기준). `domain` 은 `shared` 를 참조할 수 있으므로 방향도 맞다.

세 자리 축약(`#abc`)을 막는 것은 그대로 뒀다 — 넓히면 같은 색이 두 표기로 저장돼 문자열
비교가 어긋난다.

**정말 하나를 쓰는지 확인했다.** `isHexColor` 한 곳만 `{3,6}` 으로 느슨하게 바꾸니
`color.test.ts` · `species/rules.test.ts` · `ops/rules.test.ts` **셋이 함께** 빨개졌다.

---

## 54. 저장소 전체를 `.prettierrc` 에 맞춘다

`.prettierrc` 는 있는데 **코드가 그걸 따르고 있지 않았다.** 그래서 누가 `bun run format` 을
무심코 돌리면 **손대지도 않은 파일 수십 개가 다시 포맷돼** diff 가 폭발했다 — 실제로 스켈레톤
작업 중에 21개 파일이 그렇게 됐고, 3-way 병합으로 되돌려야 했다.

**설정을 코드에 맞추는 쪽은 안 된다.** 폭을 넓혀 봐도 어긋남이 사라지지 않는다.
...
| 100 | 161 |
| 110 | 147 |
| 120 | **154** — 되레 늘어난다. 이 폭에서는 prettier 가 손으로 끊어 둔 줄을 **합치기** 시작한다 |

폭만의 문제가 아니라 JSX 속성·객체 리터럴을 끊는 방식이 통째로 다르다. **설정이 원본이고
코드가 따라간다.**

### 54.1 ⚠️ 목 데이터 표는 자동 포맷에서 뺀다

`src/mocks/` 의 배열은 **한 줄이 한 레코드**다. 표처럼 읽는 것이 그 파일의 목적인데, 포맷하면
12칸짜리 한 줄이 **14줄**로 풀려 무엇이 무엇인지 보이지 않는다. `admins.ts` 한 파일만 해도
13줄이 145줄이 됐다.

**어느 폭에서도 터진다** — 96 이든 110 이든 같다. 그래서 폭이 아니라 `// prettier-ignore` 로
그 자리만 끈다. 표 **26곳**에 이유와 함께 붙였다.
...
⚠️ **지시자와 선언 사이에 빈 줄이나 다른 것이 끼면 안 된다.** 처음에 JSDoc 안으로 들어가거나
빈 줄이 끼어 **아무 효과가 없는 상태**로 붙었었다. 붙인 뒤 실제로 한 줄이 유지되는지 본다.

### 54.1.1 ⚠️ 마크다운은 포맷하지 않는다

prettier 는 표 칸을 공백으로 패딩해 맞추는데, **글자 수로 세고 화면 폭으로 세지 않는다.**
한글은 한 글자가 두 칸이라 **정렬이 되지도 않으면서 줄만 길어진다** — 이 문서 하나에서
600줄이 그렇게 바뀌었다. 코드와 달리 얻는 것이 없어 `.prettierignore` 에 `*.md` 와 `docs/`
를 넣었다.

### 54.1.2 ⚠️ 다른 도구가 읽는 설정도 뺀다

`.coderabbit.yaml` · `.github/ISSUE_TEMPLATE/` 에서 prettier 가 하는 일은 **큰따옴표를
작은따옴표로 바꾸는 것뿐**이다(YAML 에서는 같은 값이다). 얻는 것은 없고 잃는 것은 둘이다.

- `.coderabbit.yaml` 은 파일 머리말이 적어 두었듯 **백엔드 저장소와 같은 설정**을 쓴다.
  우리 쪽만 바꾸면 두 저장소를 대조할 때 지저분해진다. 게다가 이 파일을 건드린 PR 에는
  CodeRabbit 이 **「설정이 바뀐 PR」 경고**를 달고 대상 브랜치의 설정으로 리뷰한다 —
  실제로 이 작업의 첫 판에서 그 경고가 붙었다.
- 이슈 템플릿은 GitHub 이 읽는 폼 정의라 그쪽 문서 표기를 따르는 편이 낫다.

### 54.2 「포맷만 바뀌었다」를 어떻게 증명하나

diff 가 200개 파일이라 눈으로는 못 본다. **빌드 산출물로 잰다.**

⚠️ **청크 해시 비교는 쓸 수 없다.** 파일명이 내용 해시라 한 곳만 바뀌어도 그것을 import 하는
청크가 줄줄이 달라진다 — 손대지도 않은 recharts 청크까지 「다름」으로 나온다. **해시를 지운 뒤**
비교해야 한다.

```text
청크 103개 중 내용 동일 95개
남은 8개는 청크 경계 이동(rollup 이 모듈을 다른 청크로 옮김)
dist 총 JS  1,330,330 → 1,330,342 bytes  (+12) · 파일 수 동일
```

**+12 바이트의 정체는 `{' '}` 다.** prettier 는 JSX 를 감쌀 때 의미 있는 공백을 잃지 않으려고
명시적 공백을 넣는다.

```jsx
<b>{current}</b> / {totalPages}
→ <b>{current}</b> /{' '}
  {totalPages}
```

렌더 결과는 같다. 이 저장소에 이미 16곳이 그렇게 쓰여 있었고 5곳이 늘었다.

> ⚠️ `{' '}` 는 **줄 끝에 홀로 남는다** — 나중에 그 줄을 지우면 공백이 조용히 사라진다.

---

## 55. 접근성 검사가 상세 화면 14개를 통째로 건너뛰고 있었다

`bun run a11y` 는 「전 화면 100」 이라고 보고해 왔다. **54개 중 40개였다.**

`screenPaths()` 가 `/:` 가 든 경로를 걸러 냈다 — `/items/:itemId` 는 그대로 열 수 없으니
맞는 처리이고, 「상세를 재려면 인자로 넘겨라」(`bun run a11y /items/3`)라는 탈출구도 주석에
적혀 있었다. **그런데 아무도 안 넘겼다.**

⚠️ **탈출구를 적어 두는 것은 검사가 아니다.** 기본 실행이 조용히 74% 만 재고 있었고, 화면이
늘 때마다 그 비율이 더 줄었다. 빠진 14개가 하필 **표·다이얼로그·폼이 몰린 상세·수정 화면**
이다 — 접근성 문제가 제일 나기 쉬운 곳이다.

### 55.1 id 를 표로 두고, 없으면 멈춘다

```ts
const SAMPLE_ID: Record<string, string> = { ':itemId': '3', ':adminId': '1', … }
```

⚠️ **조용히 건너뛰지 않는다.** 파라미터에 넣을 id 가 없으면 **던진다** — 새 상세 화면을
추가한 사람이 그 자리에서 알게 된다. 안 그러면 이번 일이 그대로 반복된다.

```text
error: admindet(/admins/:adminId) 의 :adminId 에 넣을 id 가 없습니다
       — check.ts 의 SAMPLE_ID 에 추가하세요.
```

⚠️ **목이 실제로 갖고 있는 id 여야 한다.** 없는 id 를 넣으면 오류 화면이나 빈 화면을 재고
**그것을 100 점이라고 보고한다** — 같은 파일의 `VIEWER`(운영자로 재면 권한 밖 화면이
리다이렉트된다)와 똑같은 종류의 사고다. 목은 시드 RNG 라 결정적이므로 이 값들은 고정이다.

**14개가 정말 그 화면을 그리는지 브라우저로 먼저 확인했다** — 리다이렉트 없이 자기 경로에
머물고, 오류 배너도 스켈레톤도 없고, 브레드크럼이 그 화면을 가리키는 것까지 봤다.

### 55.2 결과

**54개 전부 100.** 처음 재는 14개에서도 문제가 나오지 않았다 — 숨은 빚이 없었다는 뜻이지,
검사가 필요 없었다는 뜻은 아니다. 이제부터 새 상세 화면은 자동으로 검사에 들어온다.

---

## 56. CSV 내보내기 — 「준비 중」 여섯을 연다

목록 여섯에 「CSV 내려받기 · 준비 중」 이 잠겨 있었다. TODO 는 전부 「내보내기 엔드포인트가
생기면」 이었는데, **재 보니 다섯은 이미 클라이언트에 전체가 있었다** — 파사드가
「필터에 걸린 전체」를 주고 쪽은 화면이 자른다.

| | 전체를 갖고 있나 |
|---|---|
| 회원 · 결제 · 문의 · 감사 · 쿠폰(사용 내역) | **있다** — 화면 데이터를 그대로 쓴다 |
| **아이템** | **없다** — 여기만 파사드가 쪽을 자른다(§18.4). `getItemsForExport` 를 뒀다 |

⚠️ **아이템을 화면 데이터로 만들면 12건짜리 파일이 나간다.** 「CSV 내보내기」 라는 이름과
다른 것을 주는 셈이라, 파사드에 쪽 없는 경로를 따로 뒀다. 실측으로 확인했다 —
화면 12행 · 파일 50행, 필터를 걸면 화면 4행 · 파일 4행.

> `useQuery` 로 두지 않았다. 버튼을 누른 순간에만 필요한 값이라 캐시에 얹으면 아무도 안
> 누른 목록에서도 전체를 만들어 들고 있게 된다.

### 56.1 파일의 열은 화면의 열이 아니다

표의 `COLUMNS` 는 `render` 가 JSX 를 돌려주고, 재화처럼 **두 값을 한 칸에 합쳐** 그린다.
파일은 **한 칸에 한 값**이어야 계산에 쓸 수 있어서 화면 옆에 `CSV_COLUMNS` 를 따로 둔다.

⚠️ **금액은 숫자 그대로 넣는다.** 화면의 `won()` 이 만든 「12,100원」 을 그대로 넣으면
엑셀이 **문자열로 읽어 합계가 안 나온다.**

⚠️ **열거형은 라벨을 거친다.** `PAID`·`KAKAO`·`TOSS` 가 그대로 나가면 파일을 연 운영자가
화면에서 보던 말과 다른 것을 본다. 실제로 처음엔 셋이 원시 값으로 새고 있었다.

⚠️ **없는 것을 버튼 이름으로 약속하지 않는다.** 쿠폰 상세의 TODO 는 「발급한 개별 코드」
였는데 그 목록은 목에도 계약에도 없다. **사용 이력도 「최근 8건」 이 전부다** — 카드가 그렇게
적어 두었는데도 처음엔 「사용 내역 CSV」 라고 이름 붙였다. 아이템에서 잡았던 것과 **같은
실수**다(화면에 있는 것 ≠ 이름이 약속하는 것). 지금은 「최근 N건 CSV」 이고 파일명에도
`recent` 를 넣는다 — 나중에 파일만 열었을 때도 전체가 아니라는 것이 남아야 한다.

### 56.2 ⚠️ CSV 는 형식이 셋이나 조용히 어긋난다

`shared/lib/csv.ts` 가 셋을 다 막는다. 도메인을 모르는 형식 문제라 `shared` 가 맞다.

**① 감싸기(RFC 4180).** `,`·`"`·줄바꿈이 든 칸을 감싸지 않으면 **한 칸이 여러 칸으로 갈라져
그 행부터 표가 통째로 밀린다.** 파일은 그래도 열리므로 눈으로는 「내려받아졌다」로 보인다.

**② ⚠️ 수식 주입(CSV injection).** 셀이 `=`·`+`·`-`·`@` 로 시작하면 엑셀이 **수식으로
실행한다.** 이 어드민이 내보내는 값에는 **회원 닉네임·문의 제목**처럼 사람이 적은 것이
섞여 있어서, 닉네임에 `=HYPERLINK(...)` 를 넣어 두면 그 파일을 연 **운영자의 엑셀에서
돈다.** 앞에 `'` 를 붙여 막는다.

> ⚠️ **숫자에는 붙이면 안 된다.** `-120` 도 `-` 로 시작해서 처음엔 `'-120` 이 됐는데,
> 그러면 **엑셀이 문자열로 읽어 합계에서 빠진다.** 수식 주입은 사람이 적는 **문자열**의
> 문제이지 숫자의 문제가 아니라, `typeof raw === 'number'` 면 그대로 내보낸다.

**③ ⚠️ BOM.** 없으면 **윈도 엑셀이 UTF-8 을 못 알아보고 한글이 전부 깨진다.** 이 어드민의
값은 대부분 한글이라 없으면 쓸 수 없는 파일이 된다.

> `Blob.text()` 로는 BOM 을 확인할 수 없다 — 디코딩이 먹는다. `arrayBuffer()` 로 첫 세
> 바이트가 `EF BB BF` 인지 본다.

**줄 끝은 CRLF** 다. RFC 4180 이 그렇고, 윈도 엑셀이 LF 만 있는 파일에서 줄을 합치는 일이 있다.

⚠️ **칸 안의 줄바꿈도 CRLF 로 맞춘다.** 감싸기만으로는 부족하다 — 문의 본문처럼 `\n` 이
섞인 값을 그대로 두면 파일에 두 가지 줄 끝이 섞이고, 엄격한 파서가 레코드를 잘못 나눈다.

> 이 규칙을 넣으면 **테스트 헬퍼가 먼저 깨진다.** 결과를 `\r\n` 으로 잘라 행을 세던
> 헬퍼가 **감싼 칸 안의 CRLF 까지 갈라서** 행이 늘어난다. 그 경우에는 `toCsv` 결과 전체를
> 그대로 본다.

### 56.3 검증

브라우저에서 **실제 다운로드를 일으키지 않고** 확인했다 — `URL.createObjectURL` 을 가로채
Blob 을 읽고, `a.click()` 은 파일명만 기록하게 두었다.

```text
회원   11행 12열 · 결제 13행 10열 · 문의 15행 7열 · 감사 16행 12열 · 쿠폰 9행 5열
아이템 화면 12행 → 파일 50행       (필터 걸면 화면 4행 → 파일 4행)
BOM    EF BB BF · MIME text/csv;charset=utf-8 · 파일명 riruti-<화면>-<날짜>.csv
```

회귀 주입: `csv.ts` 의 규칙 다섯(따옴표 이스케이프 · 감싸기 · 수식 방지 · `null` 처리 ·
CRLF)을 하나씩 되돌려 넣어 전부 빨개지는 것을 확인했다.

**남은 하나는 「CSV 가져오기」(레벨)다.** 파일을 읽는 것까지는 되지만 **적용에 서버가
필요해서** 잠긴 채로 둔다.
...
지표 화면의 「리포트 내보내기 · 준비 중」 을 연다. TODO 는 「서버 집계 엔드포인트가 생기면」
이었는데, **화면이 그리는 다섯이 이미 클라이언트에 있다** — KPI 6 · 일간 활성 유저 14 ·
주간 젬 7 · 인기 아이템 5 · 진행 중 챌린지 6.

> 같이 잠겨 있던 「기간 설정」 은 그대로 둔다 — **목이 14일치뿐이라 고를 것이 없다.**

### 57.1 ⚠️ 날짜 없는 시계열은 파일로 못 쓴다

`dau` 는 `{ values: number[] }` 였다. 차트는 x축에 날짜를 안 그리므로 그것으로 충분했지만,
파일에는 **어느 날의 값인지**가 있어야 한다.

화면에서 `daysAgo(13 - i)` 로 붙일 수도 있었지만 그러지 않았다 — **「몇 번째 값이 오늘인가」
는 데이터의 성질**이지 화면의 사정이 아니다. 화면이 혼자 알고 있으면 목이 15일치로 바뀌는
날 조용히 어긋난다. 파사드가 짝지어 준다.

```ts
dau: { points: { date: string; value: number }[]; domain; ticks }
```

차트는 `points.map((p) => p.value)` 로 받는다. 두 배열을 나란히 두는 것보다 **짝이 깨질 수
없는 모양**이 낫다.

실측: 14행 · 날짜 오름차순 · 끝 날짜가 오늘 · 마지막 값이 차트 끝과 같다(90).

### 57.2 ⚠️ 한 파일에 표 다섯 — 엄격한 CSV 가 아니다

구역마다 제목 줄을 두고 빈 줄로 나눈다(`csvSection` · `joinCsvSections`).
...
⚠️ **구역마다 열 수가 다르므로 파서에 그대로 물리면 깨진다.** 이건 **사람이 엑셀에서 한 번에
열어 보는 용도**다 — 도구에 물릴 값이면 `toCsv` 로 표 하나씩 내보낸다. 파일 하나로 고른
이유는 「오늘의 지표」 가 한 장으로 공유되는 것이라, 버튼 한 번에 파일 다섯이 떨어지는 것이
브라우저에서도 막히고 사람도 당황하기 때문이다.

⚠️ **구역 제목도 `toCsv` 를 거친다.** 제목에 쉼표가 들어가면 감싸야 하는데 그냥 이어 붙이면
그 줄만 규격을 벗어난다.

**타입이 구역마다 다르다.** 하나의 제네릭으로 묶이지 않아 `csvSection<T>()` 이 문자열을
만들고 `joinCsvSections` 이 잇는다 — `CsvSection<never>[]` 같은 모양은 호출부에서 터진다.

**KPI 는 이미 서식을 갖춘 문자열이라 그대로 넣는다**(`24,180`·`+6.2%`). 반대로 차트 값과
판매·달성률은 숫자로 넣는다 — 엑셀에서 계산에 쓰려면 숫자여야 한다(§56.1).

⚠️ **열거형이 또 샜다 — 세 번째다.** 챌린지의 `kind` 가 `DAILY` 로 나갔다(화면은
`CHALLENGE_KIND_LABEL` 로 「일상」 을 그린다). 앞서 아이템 `tier`·회원 `social`·결제 `pg`
에서 같은 일이 났다.

**필드를 그대로 내보내도 되는지 판별하는 기준은 「타입이 한국어 유니온인가」다.** 이
저장소는 두 방식이 섞여 있다 — `AuditKind`(`'환불' | '재화 지급' …`)·`InquiryStatus`·
`InquiryCategory`·`CouponUseLog.result` 는 값 자체가 한국어라 그대로 써도 되고,
`Tier`·`Slot`·`Social`·`Pg`·`ChallengeKind` 는 코드라 라벨 맵을 거쳐야 한다.
CSV 열을 새로 만들면 **그 필드의 타입 선언을 열어 보고** 정한다.

---

## 58. 문의 답변 임시 저장 — 폼 중 유일하게 빠져 있던 자리

폼 11곳은 이미 초안을 남기는데 **문의 답변만 예외**였다. 답변은 길게 쓰는 글이라 새로고침 한
번에 통째로 날아간다. TODO 는 「임시 저장 API 가 생기면」 이었지만, 다른 폼과 같은
`useFormDraft`(`sessionStorage`)로 지금 된다.

⚠️ **문의마다 따로 남긴다** — `inquiries:<qnaId>`. 한 칸을 나눠 쓰면 A 문의에 쓰다 만
답변이 B 문의를 열었을 때 입력창에 들어앉는다. **남의 문의에 남의 답을 보내게 된다.**

⚠️ **브라우저에만 남는다.** 다른 기기·다른 탭에서는 안 보인다 — 「쓰다 만 것을 잃지 않게」 가
목적이지 공유가 아니다. 그래서 TODO 는 지우지 않고 「서버에 남겨 기기 간에 이어 쓰게 한다」
로 남겼다.

### 58.1 ⚠️ 이 기능이 훅의 버그 둘을 드러냈다

문의 상세는 **답변을 보낸 뒤에도 화면에 그대로 머문다.** 다른 폼은 저장하면 목록으로 나가서
언마운트 정리가 뒤처리를 대신 해 주고 있었다 — 규칙이 지켜져서가 아니었다.

**① 지운 초안이 되살아난다.** `clear()` 가 **걸려 있던 저장 예약을 끄지 않았다.** 지운 뒤
500ms 가 지나면 타이머가 깨어나 `latest.current` 에 남은 **옛 값**을 다시 쓴다. 결과는
정확히 막으려던 것 — **이미 보낸 답변이 초안으로 돌아와** 다음에 열면 입력창에 앉아 있다.

**② 더러운 동안 500ms 마다 영원히 다시 쓴다.** `saveNow` 가 `savedAt` 을 바꾸면 리렌더가
일어나고, `value` 는 매 렌더 **새 객체**라 저장 효과가 다시 돈다 — 끝없는 저장 고리다.
...
훅에 예약을 담는 `ref` 를 두어 `clear()` 가 끄게 하고, **직전에 쓴 내용과 같으면 예약하지
않도록** 했다. 둘 다 `useFormDraft` 안의 문제라 **폼 11곳이 함께 고쳐진다.**

> 훅이라 단위 테스트가 없다(§17.6 — jsdom 결정이 먼저다). `sessionStorage` 쓰기를 가로채
> 세면서 브라우저에서 확인했다.

### 58.2 `key` 로 다시 마운트하지 않아도 되는 자리

§33.7 이 FAQ 에서 겪은 것과 같은 위험이 있다 — 문의를 옮겼는데 같은 인스턴스가 남으면
1번에 쓰다 만 답변이 `inquiries:2` 에 쓰인다. 실제로 `key={qnaId}` 를 넣었다가 **뺐다.**

여기서 안 나는 이유는 `KeepAlive` 의 `activeCacheKey` 가 **경로**이기 때문이다
(`AdminLayout`). `/support/inquiries/1` 과 `/2` 는 캐시 항목이 갈려 인스턴스가 애초에
섞이지 않는다. FAQ 는 **경로가 같고 쿼리만 달라서** 갈리지 않았던 것이다.

⚠️ **스켈레톤은 근거가 아니다.** `isPending` 은 캐시에 없을 때만 참이라, 30초 안에 다시
열면 스켈레톤 없이 곧장 그려진다. 「빈 칸으로 뜨더라」 만 보고 안전하다고 하면 안 된다 —
**캐시에 넣어 둔 문의로 옮겨서** 확인했다.

> 넣어도 동작은 같지만, 근거 없는 코드는 다음 사람이 지우지 못한다. 대신 **무엇에 기대고
> 있는지**를 주석으로 남겼다 — keep-alive 의 캐시 키가 바뀌면 이 자리가 깨진다.

### 58.3 검증

- 문의 1에 쓰다 → 문의 2를 열면 **빈 칸** → 1로 돌아오면 쓰던 글과 「이어서 쓰기」 알림
- 발송하면 입력칸·초안·알림이 모두 사라지고 **3초 뒤에도 되살아나지 않는다**
- 새로고침하면 살아나고, 「새로 시작」 을 누르면 초안까지 지워진다
- 저장 횟수: 고치기 전 3초에 4회 → 고친 뒤 **1회**

---

## 59. 젬 상품 등록 — 원본에 버튼만 있던 자리

원본(`riruti-admin-shop.dc.html`)은 「상품 추가」 버튼을 두고 **폼은 그리지 않았다** —
`<input>` · `<select>` · `<textarea>` 가 **하나도 없다.** 배경·업적과 같은 상황이라 같은
방식으로 그렸다(§40). 그림이 없어 에셋·업로드가 통째로 빠지고, 남는 것은 **이름 하나와
숫자 셋**이다.

**실적은 폼에 없다.** `orders` · `revenue` 는 판 결과이지 설정이 아니다 — 입력으로 받으면
운영자가 매출을 적어 넣게 되고, 그 숫자로 판매 비중(§24.2)이 계산된다. 새 상품은 둘 다
0 으로 만들어지고, 수정은 **그 자리를 건드리지 않는다**(측정: 「대형」 의 보너스를 600→900
으로 고쳐도 주간 매출 11.3백만원 · 비중 9% 가 그대로다).

### 59.1 ⚠️ 새 상품은 「예약」 으로만 만들어진다

등록 화면에는 **상태 칸이 없다.** 스토어에 상품이 없는 채로 「판매중」 을 고르면 목록에는
파는 것처럼 보이는데 **결제가 안 된다** — 원본의 부제가 「가격과 보너스는 스토어 심사 후
반영됩니다」 라고 말하는 그대로다.

만들어질 상태는 **숨기지 않고** 오른쪽 미리보기가 배지로 보여 준다. 잠긴 `select` 를 두는
쪽도 생각했지만, 고를 수 없는 컨트롤보다 「이렇게 만들어집니다」 가 짧다.

수정 화면에는 상태 칸이 **있다.** 이미 스토어에 오른 상품이고, 파는 것을 내리는 것은
운영자의 판단이다.

> 새 종을 「미출현」 으로 만드는 것과 같은 규칙이다(§34) — **아직 준비되지 않은 것이
> 그날 밤 팔리면 안 된다.**

### 59.2 ⚠️ 젬당 단가가 뒤집히면 그 상품은 죽는다

다섯은 클수록 젬당 싸다 — 11.0 → 10.3 → 9.5 → 8.5 → 8.1. 이 순서를 깬 상품은 **등록은
되지만 아무도 사지 않는다.** 같은 돈으로 더 많이 주는 팩이 바로 위에 있기 때문이다.

`cheaperBetterDeal(input, others)` 가 **더 싸면서 젬당 더 유리한** 상품을 찾아 준다.

- **막지 않는다.** 기간 한정 구성처럼 일부러 그렇게 두는 경우가 있고, 우리는 그 의도를
  모른다. 저장 전에 보여 주기만 한다.
- **파는 것만 본다.** 중단된 상품과의 역전은 뜻이 없다 — 살 수 없는 것과 비교해 봐야
  소용없다.
- **여럿이 걸리면 젬당 가장 낮은 것**을 보여 준다. 운영자가 가장 강한 반례를 봐야 한다.

⚠️ **목록 순서에 기대면 안 된다.** 「마지막에 걸린 것」 을 돌려주는 구현도 처음 쓴 시험을
통과했다 — 거기서는 답이 우연히 같았다. 순서를 뒤집는 시험을 따로 뒀다.

### 59.3 이름이 상품을 가리키는 유일한 값이다

표에 이름 말고 상품을 가리키는 열이 없다. 같은 이름이 둘이면 **어느 쪽을 고쳤는지 알 수
없고**, 판매 비중 표도 두 줄이 같은 이름으로 선다. 그래서 **중복을 막는다.**

⚠️ **목록이 없으면 검사가 조용히 꺼진다.** 빈 배열이면 이미 쓰는 이름도 통과하고, 파사드는
중복을 보지 않으므로 그대로 저장된다. 아직 오는 중이거나 실패했으면 **저장 자체를 막는다** —
검사를 못 하는 상태와 통과한 상태는 다르다(§34.1 과 같은 규칙).

⚠️ **자기 이름은 빼고 센다.** 안 빼면 수정 화면이 열리자마자 「이미 쓰고 있는 이름」 이 되어
자기 자신을 저장할 수 없다.

### 59.4 0 은 빈 칸으로 보여 준다

숫자 칸 셋을 `String(0)` 으로 그리면 **빈 폼이 「0」 셋으로 시작**해서 지우고 써야 한다.
이 셋은 앞자리가 0 인 값이 없으므로 빈 칸으로 두어도 잃는 입력이 없다.

검증은 `Number.isSafeInteger(n) && n >= 0` 하나로 **소수 · 음수 · `NaN` · `Infinity`** 를
함께 잡는다. `NaN` 은 빈 칸을 `Number()` 한 결과로 들어온다.

### 59.5 검증

- 이름 중복(` 대형 ` ↔ `대형`)이 막히고 **그 칸으로 포커스가 간다**
- 젬당 19.5원 상품을 만들면 「더 싼 중형(19,000원)이 젬당 9.5원으로 더 유리합니다」
- 등록하면 목록에 **「예약」 · 비중 0% · 매출 0.0백만원**으로 서고, 「판매 중」 은 안 는다
- 수정에서 「판매중」 으로 바꾸면 판매 중이 4→5, 비중 합은 100% 그대로
- 초안: 쓰다 나갔다 오면 되살아나고, 「새로 시작」 이 초안까지 지운다
- 미저장 탭을 닫으려 하면 「"젬 상품" 탭에 저장하지 않은 변경사항이 있습니다」
- 라이트·다크 · Lighthouse Accessibility 100 (화면 56개)

⚠️ **`Select` 는 `onMouseDown` 으로 고른다.** 자동화로 `click()` 만 보내면 값이 안 바뀌는데
**컴포넌트가 고장 난 것처럼 보인다** — 실제로 한 번 그렇게 오진했다. 브라우저 시험을 짤 때
`dispatchEvent(new MouseEvent('mousedown'))` 를 쓸 것.

### 59.6 ⚠️ `replace(/\D/g, '')` 가 오타를 열 배 값으로 만든다

숫자 칸에서 「숫자만 남기기」 를 이렇게 쓰고 있었다 — **폼 9곳, 호출부 11곳.**

```ts
Number(v.replace(/\D/g, '')) || 0
```

지우고 **이어 붙이기** 때문에 오타가 **다른 유효한 값으로 둔갑한다.**
...
`GrantsPage` 의 지급 수량, 젬 상품의 젬·가격, 배경 가격, 쿠폰 수량이 전부 이 길을 지난다 —
**`1.5` 를 친 사람이 `15` 를 지급한다.**

⚠️ **검증은 이걸 못 막는다.** `validateGemProduct` 는 소수·음수를 막지만 **그 값이 검증까지
가지 못한다.** 도메인 테스트(`gem: 1.5` → 오류)는 통과하는데 화면에서는 재현되지 않는
상태였다 — **「통과하는 테스트는 아무것도 증명하지 않는다」 의 정확한 예다**(§33.1 의
`sameShape` 가드가 같은 방식으로 새어 있었다).

`shared/lib/parseCount.ts` 로 올려 한 번에 고쳤다. **첫 번째 숫자 아닌 글자에서 자른다.**

⚠️ **`NaN` 을 돌려주지 않는다.** 통제된 입력이라 `NaN` 이면 타이핑 도중 칸이 「NaN」 으로
바뀌어 친 글자가 사라진다. 대신 못 읽은 글자를 안 받아들이고 화면이 즉시 되돌려,
「소수점은 못 쓴다」 가 눈에 보인다 — 화면과 상태가 어긋나지 않는다.

⚠️ **`OtpInput` 은 예외다.** 거기서는 이어 붙이는 것이 맞다 — 「123 456」 을 붙여 넣으면
`123456` 이어야 한다. 그 이유를 그 자리에 적어 뒀다.

### 59.7 화면이 지키는 불변식은 불변식이 아니다

「새 상품은 「예약」」(§59.1)을 **등록 화면에 상태 칸을 안 두는 것으로만** 지키고 있었다.
초안(§33.1)이 그 규칙을 통째로 지나간다 — 모양 검사는 타입만 보므로 `status: '판매중'` 이
든 초안이 폼에 들어가고, `GEM_STATUSES` 에 있는 값이라 검증도 통과한다.

```text
sessionStorage 에 {"status":"판매중"} 을 심고 등록  →  판매중으로 저장됐다
```

`asNewGemProduct()` 를 **파사드에서** 강제한다. 목 안이 아니라 `USE_MOCK` 분기 **밖**이다 —
「스토어 상품 id 가 없다」 를 아는 자리가 거기이고, 목 안에 두면 실서버로 바꿀 때 같이
사라진다.

**화면도 「저장될 값」 을 그린다.** 처음에는 폼 값을 그대로 그려서, 저장은 「예약」 인데
**미리보기 배지는 「판매중」** 이라고 말했다. 체크리스트가 검증 결과를 그대로 쓰는 것과 같은
규칙이다(§18.7) — **보여 주는 것과 저장되는 것이 갈리면 안 된다.**

> 같은 자리에서 `useFormDraft` 의 `written.current` 를 `setItem` **뒤에** 옮겼다. 저장이
> 실패했는데 「이미 썼다」 로 남는 것은 틀렸다. 브라우저에서 되돌려 봤을 때는 재시도가
> 그대로 돼서 **관측되는 고장을 못 찾았다.**
>
> ⚠️ **그 판단은 틀렸다.** 나중에 컴포넌트 테스트를 들이고 같은 회귀를 주입하니
> **잡혔다**(§60.2). 브라우저에서 **값을 바꿔 가며** 시험한 것이 문제였다 — 값이 다르면
> 어차피 새 예약이 걸린다. **갈리는 것은 값이 그대로일 때뿐**이고, 그 경우 저장이 실패한
> 값이 「이미 썼다」 로 남아 **다시는 안 써진다.** 손으로 하는 확인이 「안 난다」 를
> 증명하지 못한다는 것을 이 항목이 그대로 보여 준다.

---

## 60. 컴포넌트 테스트 — 미뤄 뒀던 경계를 열었다

`vite.config.ts` 의 `include` 가 `.test.ts` 만 잡고 있었고, 그 주석이 **결정을 미뤄 둔
것**이라고 말하고 있었다. 미뤄 둔 대가를 두 PR 이 그대로 치렀다.

| 버그 | 어떻게 찾았나 |
|---|---|
| `useFormDraft` 무한 저장 고리 | 브라우저 콘솔에서 `setItem` 을 세어서 |
| `clear()` 가 예약을 안 꺼 **보낸 답변이 되살아남** | `Storage.prototype` 를 가로채서 |
| `written` 순서 | 브라우저에서 `QuotaExceededError` 를 던지게 만들어 — **재현 실패** |
| `OtpInput` 이 로그인을 조용히 삼킴 | 손으로 재현 |

세 번째가 핵심이다. **손으로 하는 확인은 「났다」 는 보일 수 있어도 「안 난다」 는 증명하지
못한다.** 그리고 실제로 그 판단이 틀렸다는 것이 이 작업에서 드러났다(§60.2).

### 60.1 ⚠️ 환경을 전역으로 바꾸지 않는다

`domain/` 이 `node` 인 것은 의도다. 전역을 jsdom 으로 돌리면 **순수 층 테스트 700여 개가
이유 없이 브라우저를 흉내 낸다.** DOM 이 필요한 파일이 **첫 줄에 스스로 선언한다.**

```ts
// @vitest-environment jsdom
```

설정에 glob 을 두는 쪽(`environmentMatchGlobs`)은 vitest 4에서 빠지기도 했지만, 그보다
**「어느 파일이 DOM 을 쓰는가」가 파일 밖에 숨는 것**이 문제다.

### 60.2 검증은 회귀 주입이다 — **열두 개를 넣어 봤다**

| 되돌린 것 | 잡혔나 |
|---|---|
| `written` 비교 제거 (무한 고리) · `clear()` 가 예약 안 끔 · **`written` 을 `setItem` 앞에서 갱신** · 언마운트 정리 제거 · `dirty` 무시 · `scope` 무시 | 6/6 |
| `OtpInput`: 같은 값 가드 제거 · 자릿수 자르기 제거 · 숫자만 남기기 제거 | 3/3 |
| `focusFirstError`: 마지막 오류로 감 · 문서 전체에서 찾음 · `aria-invalid` 값 무시 | 3/3 |

⚠️ **굵게 표시한 것이 이 작업의 값이다.** 브라우저에서 「관측되는 고장을 못 찾았다」 고
적었던(§59.7) 바로 그 회귀를 테스트가 잡았다.

> 검증 스크립트 자체가 한 번 고장 났다. `vitest run 'src/shared/**/X.test.tsx'` 가 아무
> 파일도 못 찾아 **여섯 개가 전부 「안 잡힘」 으로 나왔다.** 「전부 안 잡힌다」 는 결과는
> 대개 시험 대상이 아니라 **시험 장치가 고장 났다는 뜻**이다.

### 60.2.1 ⚠️ `toHaveBeenCalledWith` 는 횟수를 안 본다

리뷰에서 나왔다. **같은 인수로 두 번 불러도 통과한다.** `OtpInput` 의 시험 이름이
「다 차는 그 입력에서 **한 번** 부른다」 인데 정작 횟수를 재지 않고 있었다 —
**시험 이름이 검증하지 않는 것을 주장하고 있었다.**

하필 이 컴포넌트에서 실제로 났던 버그가 **헛되이 부른 것**(로그인 삼킴)이라, 여기서는
「무엇으로 불렀나」 보다 **「몇 번 불렀나」** 가 더 중요하다. 일곱 곳을 `calledOnceWith`
헬퍼로 바꿨다.

회귀 주입: `onChange` 를 두 번 부르게 하면 **5건**, `onComplete` 를 두 번 부르게 하면
**1건** 빨개진다 — **고치기 전에는 둘 다 하나도 안 잡혔다.**

### 60.3 ⚠️ 밟은 함정 셋

**① `bun run test` 가 `panda codegen` 을 안 돌았다.** 컴포넌트 테스트는
`styled-system/css` 를 타는데 그건 **생성물이라 깨끗한 클론에 없다** — 붙이지 않으면 새
머신에서 `bun run test` 가 통째로 깨진다. `typecheck` 와 같은 이유로 앞에 붙였다.

**② Testing Library 의 자동 정리가 안 걸린다.** 그쪽은 **전역 `afterEach` 가 있을 때만**
스스로 등록하는데, 우리는 `globals` 를 켜지 않는다. 실측: `render` 를 두 번 하면
`document.body` 의 `<button>` 이 **2개**가 됐다. `vitest.setup.ts` 에서 `afterEach(cleanup)`.

걷지 않으면 앞 테스트의 DOM 이 남아, `document` 전체를 뒤지는 코드가 **엉뚱한 요소를 잡고도
초록이 된다.**

**③ ESLint 의 vitest 금지 예외가 `.test.ts` 만이었다.** 프로덕션 코드가 `vitest` 를 import
하는 것을 막는 규칙인데(§17.6), 예외 목록에 `.tsx` 가 없어 **컴포넌트 테스트가 전부 lint
에러**가 된다.

### 60.4 넣은 것과 안 넣은 것

**둘만 넣었다** — `jsdom` · `@testing-library/react`.

`user-event` 와 `jest-dom` 은 안 넣었다. `fireEvent` 와 `expect(el.value)` 로 지금 목록이
다 커버되고, 이 저장소 규칙이 **「두 번째 사용처가 생길 때 올린다」** 이기 때문이다(§4.4).

`download.ts` 는 테스트를 안 썼다. jsdom 에 `URL.createObjectURL` 이 없어 스텁을 짜야 하는데
**그러면 스텁이 스텁을 테스트한다.** 실제 다운로드는 브라우저에서만 의미가 있다.
...
⚠️ **따옴표를 두르지 않는다.** `Pretendard` 는 공백이 없어 CSS 문법상 필요가 없고, 토큰이
**필요할 때만** 감싸는 것(`Pretendard` 는 맨몸, `'Apple SD Gothic Neo'` 만 감쌈)과 표기를
맞춘다. 동작은 같지만 한 이름이 두 표기로 저장소에 남으면 검색이 갈린다.
...
## 62. 하드코딩 색 — `strictTokens` 대신 검사기

미결이던 「`strictTokens: true` 도입 여부」를 **안 켜는 쪽으로 닫는다.** 대신 목적(색을
박아 넣지 못하게)을 `check-tokens.ts` 가 맡는다.

### 62.1 ⚠️ 켜 보니 1,765건이고, 대부분 색이 아니었다

`strictTokens` 는 **색만 강제하는 스위치가 아니다.** 실제로 켜서 재 봤다.

```text
Type '"10px"' is not assignable to type
  'ConditionalValue<WithEscapeHatch<SpacingToken | `var(--${string})`>>'
```

걸린 것의 대부분이 `SpacingToken` — `gap: '13px'` · `p: '17px 20px'` 같은 **간격**이다.
이 저장소는 **디자인 원본의 px 값을 그대로 옮기는 것이 규약**이고(§3.2), 원본은 8px 그리드가
아니다. 토큰으로 강제하면 **디자인이 바뀐다.**

> 대괄호 이스케이프(`p: '[17px]'`)로 통과시킬 수는 있지만, 1,765곳을 그렇게 바꾸면 규칙이
> 아니라 **잡음**이 된다.

### 62.2 ⚠️ 색을 박으면 명암비 검사가 못 본다

`check-contrast.ts` 는 **토큰만** 잰다(§3.5). 그래서 코드에 박힌 색은 **아무도 재지 않는다.**
실제로 새 있었다.

| 자리 | 라이트 대비 | |
|---|---|---|
| `ProgressBar` 의 60%↑ (`gFg`) | 4.88 | ✓ |
| `ProgressBar` 의 35%↑ (`pri`) | 4.35 | ✓ |
| **`ProgressBar` 의 35%↓ (`'#E8A33D'`)** | **2.03** | ✗ 비텍스트 3:1 미달 |

**진행률이 낮을수록 눈에 띄어야 하는 자리인데 오히려 안 보였다.** 원본도
`rate >= 35 ? C.pri : '#E8A33D'` 로 **그 색만 값으로 박아** 두었고(원본에 이름이 없다),
그대로 옮긴 결과다. 다크에서는 8.41 로 멀쩡해서 더 눈치채기 어려웠다.

경고 톤 토큰 `aFg` 로 바꿨다 — 라이트 **5.10** · 다크 **8.37**. 덤으로 **테마를 따르게**
됐다(전에는 고정값이라 다크에서도 같은 주황이었다).

같은 검사가 **딤 오버레이 둘이 갈려 있던 것**도 드러냈다 — `Dialog` 는 `.42`,
`CommandPalette` 는 `.32`. 값은 유지하되(지금 화면이 그렇다) `dim`·`dimSoft` 토큰으로 올렸다.
원본에는 둘 다 없다(원본은 `.06`·`.14`·`.24` 만 쓴다).

### 62.3 ⚠️ 데이터 층의 색은 검사하지 않는다

`mocks/species.ts` 의 종 대표 색 13개, `mocks/ops.ts` 의 이벤트 강조색 6개는
**운영자가 정해 서버에 저장하는 값**이다. 토큰이 될 수 없고 테마를 따라서도 안 된다.

처음에 전 파일을 검사했더니 **25건이 나왔고 그중 19건이 그것**이었다. 화면을 그리는 층
(`features` · `shared` · `layouts` · `entities` · `app`)만 본다.

같은 이유로 `placeholder="#2F7CEF"` 도 뺀다 — **운영자에게 보여 주는 예시 글자**이지
칠하는 색이 아니다. ⚠️ **오탐이 섞이면 사람이 검사를 끄고, 그러면 없느니만 못하다**(§39.2).

### 62.4 예외는 **값 단위**로 등록한다

토큰을 안 쓰는 것이 맞는 자리가 있다 — QR(테마를 따르면 스캐너가 못 읽는다) · 브랜드
그라디언트 위의 흰 글자 · 운영자가 고르는 색의 기본값.

⚠️ **파일 단위로 풀면 그 파일의 「다른」 하드코딩까지 조용히 통과한다.** 실측으로 확인했다 —
`isAllowed` 를 파일 단위로 바꾸고 `BrandPanel` 에 `#123456` 을 심으니 **안 잡혔다.**

예외마다 **이유를 함께 적는다.** 그게 이 목록의 요점이다 — 「토큰을 안 쓴다」 는 판단이
리뷰에 걸리게 만드는 것.
...
| 되돌린 것 | |
|---|---|
| **주황을 하드코딩으로** (실제 결함) | 위반 1건 |
| 딤을 `rgba` 로 | 위반 1건 |
| 예외 파일의 **다른** 색 | 위반 1건 |
| 새 파일에 3자리 hex (`#f00`) | 위반 1건 |
| 화면 파일에 `rgba()` | 위반 1건 |

⚠️ **처음에는 첫 줄이 안 잡혔다.** 검사기가 `css()` 안만 보는데 그 색은 **함수 반환값**
이었다 — `ProgressBar` 는 `style={{ background: barColor(…) }}` 로 넘긴다. **이 검사기를
만든 이유가 그 결함인데 정작 그것만 못 잡고 있었다.** 스타일 함수 밖도 보게 넓혔다.

브라우저 실측: 라이트에서 낮은 진행률 막대가 `rgb(138,99,20)`(`aFg` base), 다크에서
`rgb(224,166,60)`(`aFg` dark) — **테마를 따른다.** 딤은 `rgba(16,24,40,0.32)` 그대로.

### 62.6 ⚠️ 검사기가 뚫려 있던 네 경로

리뷰에서 나왔다. 만들어 놓고 **실제로 뚫어 보지 않은 자리**가 있었다.

| 심어 본 것 | 고치기 전 | 고친 뒤 |
|---|---|---|
| `'RGBA(1,2,3,.5)'` | **안 잡힘** | 위반 1건 |
| `'HSL(1,2%,3%)'` | **안 잡힘** | 위반 1건 |
| `` `#f00` `` (백틱) | **안 잡힘** | 위반 1건 |
| `` `rgba(1,2,3,.5)` `` (백틱) | **안 잡힘** | 위반 1건 |

**① 대소문자.** hex 는 `[0-9A-Fa-f]` 로 양쪽을 봤는데 **함수 이름만 소문자**였다.
`RGB(` 는 CSS 에서 유효하다 — 플래그 하나(`/i`)가 빠져 있었다.

**② 백틱도 문자열이다.** `` `#f00` `` 은 `StringLiteral` 이 아니라
`NoSubstitutionTemplateLiteral` 이라, **따옴표를 백틱으로 바꾸는 것만으로 검사를
통과했다.** 판별을 `asText()` 한 곳으로 모아 둘을 같이 본다.

⚠️ **치환이 든 템플릿(`` `#${x}` ``)은 대상이 아니다.** 값이 실행 시점에 정해지므로 여기서
잴 수 없다 — `BrandPanel` 의 `tint` 가 그 모양이라(`` `${t}${o})` ``) 잡으면 오탐이 된다.
확인했다: 치환 템플릿 둘을 심어도 **안 잡는다.**

> 이 PR 에서만 검사기가 **두 번** 뚫렸다 — 처음에는 함수 반환값을 못 봤고(§62.5),
> 그다음이 이것이다. **검사기를 만들면 그 검사기도 뚫어 봐야 한다.**

---

## 63. 키보드 순회 점검 — 자동 검사가 못 보는 자리

Lighthouse a11y 는 **포커스 순서·키보드 트랩·스크린리더 읽기 순서를 아예 검사하지 않는다**
(§28). 56화면 100 을 유지하면서도 그 셋은 아무도 안 본 상태였다. 손으로 훑었다.

**멀쩡했던 것** — 포커스 링(`:focus-visible` + `ring` 토큰) · `Dialog` 의 트랩과 복귀 ·
탭 전환 시 포커스 유지 · 표 행의 Enter/Space(`Table` 이 이미 처리) · `Select` 의 ↑↓/Esc.

**고친 것 둘**이 아래다.

### 63.1 ⚠️ `CommandPalette` 를 닫으면 포커스를 잃었다

Esc 로 닫은 뒤 포커스가 **`body` 로 떨어졌다.** 키보드로 쓰는 사람은 팔레트를 닫을 때마다
**본문으로 돌아가려고 Tab 을 17번** 눌러야 했다.

원인은 **닫는 방식**이다.

| | 닫는 법 | 포커스 |
|---|---|---|
| `Dialog` | `el.close()` | **돌아온다** — 브라우저가 열기 전 자리를 기억한다 |
| `CommandPalette` | 언마운트(DOM 에서 제거) | **`body` 로 떨어진다** |

네이티브 `<dialog>` 의 포커스 복귀는 **`close()` 를 불렀을 때만** 일어난다. DOM 에서 그냥
빼면 top layer 는 거둬도 포커스는 안 돌려준다. 예전 주석이 「DOM 에서 빠지면 브라우저가
top layer 도 같이 거둔다」 라고 한 것은 맞지만, **포커스까지는 아니었다.**

⚠️ **`focus()` 만 불러서는 안 된다.** 언마운트 정리는 **DOM 제거 전에** 도는데, 그때
`<dialog>` 는 아직 top layer 에 있어 **바깥 요소로 포커스를 줄 수 없다** — 실측으로
확인했다(그냥 `focus()` 하면 여전히 `body`). `close()` 를 **먼저** 부른다.

⚠️ **창을 창 안에 그리면 Esc 가 둘을 닫는다.** 안쪽 `<dialog>` 가 바깥 `<dialog>` 의
**DOM 자식**이라, React 의 합성 `onCancel` 이 위로 올라가 **한 번에 두 창이 닫히고 포커스는
`body` 로 떨어진다.** 고르던 자리를 통째로 잃는다 — `Dialog` 가 `stopPropagation` 을
부른다. 실측으로 확인했다(고치기 전 2 → 0, 고친 뒤 2 → 1 이고 포커스는 부른 버튼으로).

⚠️ **화면을 옮겼으면 옛 자리로 되돌리지 않는다.** 처음에는 「이동하면 그 요소는 이미
사라졌다(`isConnected` 가 거짓)」 고 적었는데 **틀렸다** — §63.4 를 볼 것.

### 63.2 본문 바로가기가 없었다 — 본문까지 Tab 17번

`/items` 에서 실측: 사이드바 그룹 8 + 프로필 + 로그아웃 + 브레드크럼 2 + 검색 + 테마 +
탭 2 + 배너 1 = **17번**을 눌러야 본문 첫 요소에 닿는다. **화면을 옮길 때마다 반복한다.**

`SkipLink` 를 DOM 맨 앞에 뒀다 — **Tab 1번 → Enter → Tab 1번**으로 본문에 닿는다.

⚠️ **`<main>` 에 `tabIndex={-1}` 이 필요하다.** 없으면 `#main-content` 로 가도 **주소만
바뀌고 포커스는 안 옮겨진다.** 음수 tabindex 는 **Tab 순서에는 안 들어가면서** 스크립트·
프래그먼트로는 포커스를 받게 한다.

⚠️ **`display: none` 으로 숨기면 안 된다** — 포커스를 못 받아 **있으나 마나**가 된다.
화면 위로 밀어 두고(`top: -100px`) 포커스를 받으면 내려온다.

⚠️ **`_focus` 로 바꾸지 말 것.** Panda 에서 `_focus` 의 `top` 은 기본값에 **밀려 적용되지
않는다** — 클래스는 생기는데 화면 밖에 남는다. `_focusVisible` 은 이긴다(실측). 이 링크는
Tab 으로만 오므로 `:focus-visible` 로 충분하다.

> ⚠️ **시험 방법 때문에 두 번 헛짚었다.** `:focus-visible` 은 브라우저가 「키보드로
> 왔는가」 를 판단해 켜므로, **테마 버튼을 클릭한 뒤** `link.focus()` 로 재면 안 나타난다 —
> 그걸 보고 `_focus` 로 바꿨다가 Panda 우선순위에 막혀 되돌렸다. 키보드 모드를 만든 뒤
> 재니 라이트·다크 **둘 다 `top: 8` 로 내려온다.** 브라우저 자동화로 포커스 스타일을 잴
> 때는 **직전에 실제 키를 한 번 눌러 두어야 한다.**

### 63.3 검증

- 회귀 넷을 주입해 `SkipLink.test.tsx` 가 전부 잡는 것을 확인 — href 를 다른 곳으로 ·
  `display: none` · `hidden` 속성 · `tabIndex={-1}`
- ⚠️ **처음 쓴 단언은 `display: none` 을 못 잡았다.** `css()` 는 클래스를 만들지
  인라인 `style` 을 쓰지 않아 jsdom 에서 `el.style.display` 가 언제나 빈 문자열이다 —
  **주입해도 초록이었다.** 클래스 기반으로 고쳤다.
- 브라우저 실측: Esc 뒤 포커스가 열기 전 버튼으로 복귀 · 본문까지 17 → 2 ·
  라이트·다크 모두 `top: 8`

### 63.4 ⚠️ 셸은 라우트가 바뀌어도 남는다

리뷰에서 나왔다. §63.1 을 고치면서 「이동하면 그 요소는 이미 사라졌으니 `isConnected` 만
보면 된다」 고 적었는데, **팔레트를 연 검색 버튼은 `Topbar` 에 있다.** 셸은 라우트가 바뀌어도
살아 있어서 `isConnected` 가 참이다.

```text
/items 에서 팔레트로 「업적 목록」 선택  →  /achievements 로 이동
                                          포커스는 **상단 검색 버튼**   ✗
```

새 화면에 왔는데 포커스가 헤더에 있다. 실측으로 재현했다.

**고른 답은 「복귀하지 않는다」 가 아니다.** 그러면 `body` 로 떨어져 처음부터 Tab 해야 한다.
**가려던 화면의 `<main>` 으로 보낸다** — 화면을 고른 사람의 다음 동작은 그 화면을 쓰는
것이고, 포커스를 옮길 곳은 `SkipLink` 가 이미 만들어 둔 `MAIN_ID` 다.

| 닫힌 경로 | 포커스 |
|---|---|
| Esc · 배경 클릭 | 열기 전 자리(트리거) |
| **화면 선택(이동)** | **새 화면의 `<main>`** |

### 63.5 이 회귀는 jsdom 으로 못 잡는다

리뷰가 회귀 테스트를 요구했는데 **여기서는 못 쓴다.** jsdom 에 `HTMLDialogElement.showModal`
이 없다(실측: `typeof d.showModal === 'undefined'`). 스텁을 넣을 수는 있지만 이 동작의 핵심이
**top layer 가 포커스를 가둔다**는 브라우저 규칙이라, 그걸 흉내 낸 스텁은 **스텁을 테스트하는
것**이 된다 — `download.ts` 를 테스트하지 않은 것과 같은 판단(§60.4).
...
## 65. AI 심사 차트 — 눈으로 본 것이 틀렸다

### 65.1 예측이 두 번 틀렸다 — 그래서 재고 시작했다

「첫 막대가 잘려 있을 것」(§26.4 재발)이라고 봤는데 **틀렸다.** 헤드리스 크롬으로 재니
막대는 28개(14일 × 2계열)가 전부 있었고 첫 막대의 `x` 는 14.3 이었다.

**대신 음수 여백은 진짜 문제였고, 증상이 반대쪽이었다.**

```text
SVG 폭 1104 · overflow: hidden
  60%  라벨 상자 1088 → 1108.7   (4.7px 밖)
 100%  라벨 상자 1088 → 1112.9   (8.9px 밖 = `%` 글자 폭)
```

화면에는 **「100%」 가 「100」 으로** 나오고 있었다. `margin.right: -14` 가 그림 영역을
오른쪽으로 밀어 축 라벨을 SVG 밖으로 내보낸 것이다 — §26.4 가 왼쪽에서 잡은 것과 같은 원인,
반대편이다.

> **눈으로 본 것을 근거로 삼지 않는다.** 이 절의 사실은 전부 `getBoundingClientRect` 로
> 잰 값이다. 「잘려 보인다」 는 인상은 틀렸고, 재고 나서야 진짜가 나왔다.
>
> 재는 도구는 이미 있었다 — `scripts/a11y` 의 puppeteer 를 그대로 쓴다. 접근성 검사용으로
> 들인 것이지만 **화면을 실제로 띄워 좌표를 재는 일**에는 다 쓸 수 있다.

### 65.2 그림은 있는데 읽을 수가 없었다

| | 고치기 전 | 고친 뒤 |
|---|---|---|
| 건수 축 | `hide` — 막대가 50건인지 500건인지 모른다 | 0 · 중간 · 상한 세 눈금 |
| 값 확인 | 없음 | 말풍선 (날짜 · 승인 · 반려 · 합계 · 통과율) |
| 축 상한 | 데이터 최댓값 그대로 — **최고 막대가 천장에 닿았다** | 어림수로 한 단계 위 (12% 여유) |
| 반려 색 | `aFg` — **앰버 글자색**을 큰 면적에 칠했다 | `chart2` (칠하는 색) |
| 범례 조각 | 투명 — 아래 §65.3 | 색 + 테두리 |
| 모서리 | 위 계열만 둥글어 **반려 0인 날만 각졌다** | 평평하게 통일 |

가로 그리드선이 **맨 위·맨 아래 둘뿐**이었던 것도 같은 뿌리다. 읽을 눈금이 없는 선은
장식이고, 눈금을 붙이자 선이 뜻을 갖는다.

⚠️ **`aFg` 를 막대에 칠하지 말 것.** 그것은 `aBg` 위에 얹는 **글자색**이라, 큰 면적으로
칠하면 탁한 갈색 덩어리가 된다. `chart2` 는 **칠하는 색**이고 비텍스트 3:1 을 넘긴다
(라이트 3.42:1 · 다크 6.30:1).

### 65.3 ⚠️ 이름은 맞는데 **규칙이 없는** CSS

범례의 「승인」 조각이 안 보였다. 옅은 색이라 묻힌 줄 알았는데, 재 보니
`background-color: rgba(0, 0, 0, 0)` — **투명**이었다.

```text
class="… bg_chart …"     ← 클래스 이름은 나갔다
.bg_chart { … }          ← 그런 규칙이 없다
```
...
```tsx
;['chart', 'aFg', 'pri'].map((tone) => <span className={css({ bg: tone })} />)
```

Panda 는 정적으로 읽어 CSS 를 만든다. `bg: tone` 의 `tone` 은 실행 시점에 정해지므로
**규칙이 생성되지 않는데, 클래스 이름은 런타임에 그대로 붙는다.** `bg: 'pri'` 는 저장소
어딘가에 글자 그대로 있어서 규칙이 있었고, `bg: 'chart'` 는 한 번도 없어서 없었다 —
**세 조각 중 둘만 보이던 이유가 이것이다.**

§39 의 「없는 토큰 이름」 과는 다른 구멍이다. 저쪽은 *이름이 틀린* 경우라
`check-tokens` 가 잡지만, 이쪽은 **이름이 맞다.** 그래서 안 잡힌다.

**색은 `style` 로 넣는다.** 막대·선이 이미 `token('colors.x')` 를 쓰므로 범례·말풍선도
같은 값을 보게 한 자리(`SERIES_FILL`)로 모았다 — 네 곳에 따로 적으면 하나만 바뀌어도
범례가 거짓말을 한다.

> 저장소 전체를 훑어 **같은 모양은 이 한 곳뿐이었다.** 다른 자리는 전부 글자 그대로 쓴다.

---

## 66. 인증 사진 열람 — 없는 기능이 아니라 안 옮겨온 패턴

「보기 · 준비 중」 으로 잠겨 있었고 `TODO(사진 열람 API 가 생기면)` 이 걸려 있었다.
그런데 **신고 처리 화면이 같은 상황을 이미 풀어 놨다** — 사진 원본이 없어도 §23.5 대로
**자리와 취급 규칙**을 그린다. API 를 기다릴 이유가 없었다.

그 블록을 `entities/moderation/CertPhotoPanel` 로 올렸다 — 두 번째 소비자에서 승격(§4.4).
신고는 **올라온 뒤**, AI 는 **올라오는 시점**을 보지만 **취급 규칙은 시점과 무관하게 같다.**

⚠️ **확인할 것이 없는 창에 확인 버튼을 두지 않는다.** `Dialog` 의 `onConfirm` 을 선택으로
바꿔, 안 주면 버튼이 「닫기」 하나로 줄어든다. 「취소 / 확인」 이 나란히 있으면 무언가
결정하라는 뜻으로 읽히는데, 사진을 들여다보는 창에는 결정할 것이 없다.

행마다 버튼 이름이 「보기」 로 같으면 스크린리더로 훑을 때 어느 행인지 알 수 없어,
보이는 글자는 두고 접근 이름만 늘렸다 — `aria-label="풀잎 의 아침 6시 기상 인증 보기"`(§37).

**반려는 여전히 열 수 없다.** 기록이 남지 않아 목록에 나타나지도 않는다(§23.8) — 화면
아래 경고가 그대로 있는 이유다.

⚠️ **「보기 버튼이 셀을 넘쳐 잘린다」 는 예측은 틀렸다.** 재니 셀 124.3px 에 버튼 96.3px 로
여유가 있었다 — 스크린샷을 잘못 잘라 놓고 잘린 줄 알았다. §65.1 과 같은 교훈이다.
