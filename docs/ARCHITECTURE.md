...
세 가지가 조용히 깨질 수 있어 각각 막아 뒀다.

**① 다 찬 뒤에 더 눌러도 다시 검증하면 안 된다.** 7번째 숫자는 잘려서 값이 그대로인데,
그걸로도 부르면 **누를 때마다 같은 코드를 다시 보낸다.** `next !== value` 로 막는다 —
빼서 넣어 보니 추가 입력 세 번에 재검증 2회가 났다(세 번째는 아래 ②가 막았다).
