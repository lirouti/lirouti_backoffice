...
**맞았는지는 재서 확인한다.** 배경 목록에서 스켈레톤 카드와 실제 카드를 나란히 재면
`7열 191×239` 대 `7열 191×249` 로, 열 수와 폭이 같고 높이만 10px 다르다. 반대로 `min` 을
190 → 300 으로 되돌려 넣으면 **4열 344px 대 7열 191px** 로 벌어진다 — 규칙이 실제로 일하고
있다는 것을 이렇게 확인했다.
