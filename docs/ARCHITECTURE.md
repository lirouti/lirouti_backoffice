...
⚠️ **원본 설계에서 한 가지를 뒤집었다.** 원본은 탭 단위가 **섹션**(`sectionOf`)이라
상세로 들어가도 탭이 늘지 않았다. 그 화면들은 상태가 살아 있지 않았기 때문에
가능한 설계였는데, 작성·수정이 대부분인 실제 요구에는 맞지 않는다 —
`/items/3` 과 `/items/7` 을 동시에 편집할 수 없다.

- 탭 단위는 **URL** 이다. `/items/3` 과 `/items/7` 은 **별개 탭**으로 열린다.
- `sectionOf` 는 여전히 쓰인다 — **사이드바 활성 표시와 브레드크럼에서만**. 탭은 안 쓴다.
- 최대 **12개**(`MAX_TABS`). `tabsStore` 와 `<KeepAlive max>` 가 **같은 값**이어야 한다 —
  스트립 길이 제한이자 **메모리 상한**이다.
- 넘치면 **깨끗한 탭부터** 밀어낸다. 미저장 탭을 말없이 버리면 작성 중이던 내용이 사라진다.
- 활성 탭을 닫으면 마지막 탭으로 이동. 마지막 탭을 닫으면 목록만 비운다.
- 권한 밖(`canAccess === false`) 탭은 복원 시 걸러낸다.

**열린 탭의 화면은 언마운트되지 않는다.** `<KeepAlive>` 가 `<Outlet/>` 을 감싸고,
...
| 친 것 | 사라진 것 | 원인 |
|---|---|---|
| `왕실 벨벳` | 가운데 공백 | `parse` 가 `trim()` 했다 |
| `_왕실 벨벳` | **앞** 공백 | 공백뿐인 값이 주소에 안 실려 첫 글자가 증발 |
| (검색어 지우려고) `___` | 친 공백 전부 | 우리가 쓴 값과 주소에서 돌아온 값이 달랐다 |

**초안은 화면이 들고, 주소에는 조용해졌을 때만 쓴다**(`useSearchDraft`, 250ms).
곁들여 매 타건마다 라우터를 다시 그리는 것도 없어진다. 다듬는 것은 **주소에 쓸 때**와
**거를 때**(`filterItems`)만 한다.

초안과 주소를 잇는 규칙 둘 — 둘 다 위 표의 세 번째 줄을 막는다.

- **다듬어서 같으면 주소에 쓰지 않는다.** `'   '` 과 `''` 는 검색어로서 같다.
- **다듬어서 같으면 초안도 덮지 않는다.** 밖에서 온 변경(뒤로가기·초기화·링크)만 따른다.
