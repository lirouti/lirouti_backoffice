...
**남는 구멍 하나는 알고 남긴다.** 상태 가드는 React 가 「진행 중」을 커밋한 뒤부터 듣는다.
**같은 태스크에서 동기로 연달아** 제출하면 두 번째가 아직 옛 값을 본다 — 실측으로 3회 중
2개가 만들어졌다. 50ms 만 띄우면 **1개**다.

| 간격 | 만들어진 수 |
|---|---|
| 동기 연타 (같은 태스크) | 2 / 3 |
| 50ms | **1 / 3** |
| 업로드 중(500ms) 재제출 | **1 / 3** · 버튼도 「저장 중…」 으로 잠긴다 |

키 반복도 더블클릭도 태스크가 나뉘므로 **사람이 낼 수 있는 간격은 전부 막힌다.** `useRef`
로 완전히 닫을 수도 있지만, 초기화를 한 경로(업로드 실패·저장 실패)에서라도 빠뜨리면
**폼이 영영 잠긴다** — 막으려던 것보다 나쁜 고장이다. 중복의 마지막 방어선은 원래 화면이
아니라 **파사드와 서버**다(§22.2.3).
