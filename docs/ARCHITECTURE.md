...
**규칙**: 화면이 헤더를 먼저 그리고 **본문만** 스켈레톤으로 채운다.

⚠️ **아직 전부가 그렇지는 않다.** `return <Skeleton />` 로 화면을 통째로 갈아 끼우는 화면이
**28개 남아 있다** — 제목·부제·버튼이 전부 사라졌다가 한꺼번에 나타나서 **아는 것까지 튄다.**
지금 이 규칙을 지키는 것은 지표 화면과, 원래부터 헤더를 유지하던 목록 화면 16개다.

남은 28개는 화면마다 「무엇이 고정이고 무엇이 데이터인가」를 따로 판단해야 해서 뒤로 뺐다.
**끝날 때까지 이 절은 「목표」이지 「현황」이 아니다.**

⚠️ **제목이 불러온 값이면 그건 못 그린다.** 상세 화면의 제목이 「소이」 처럼 데이터에서 오면
그 자리는 막대로 두고, 부제·버튼처럼 **고정인 것만** 그린다. 아는 것과 모르는 것을 섞지 않는다.
