...
- 탭 라벨은 서브 메뉴 이름이고, 파생 화면에 있으면 `›` 로 덧붙인다
  (`아이템 목록 › 후드`) — 탭이 안 늘어나는 대신 **탭만 봐도 상세에 있는지 알아야** 한다.
...
상세를 보다 목록으로 돌아왔을 때 필터·스크롤이 그대로여야 "탭 안에서 전환됐다"로 읽힌다.
그래서 keep-alive 상한은 `MAX_TABS * 2` 다. 다른 상세로 옮기면 앞의 상세는 파기된다 —
같은 서브 메뉴의 상세를 둘 동시에 살려 두지 않는다.
