...
- **글롭·자리표시자**(`src/api/*.ts` · `features/<name>/`)와 **디렉터리**는 경로 검사에서 뺀다 —
  `src/entities/` 처럼 **아직 없는 것을 가리키는** 경우가 정상적으로 있다
...
「열람 기록은 감사 로그에 남습니다」 는 지금 **사실이 아니다** — 감사 로그 API 가 없다. 그래도 남긴 이유는 이게 화면의 상태가 아니라 **운영 규칙**이고, 규칙을 먼저 적어 두지 않으면 API 가 붙을 때 아무도 이 요구를 기억하지 못하기 때문이다. `api/moderation.ts` 에 `TODO(감사 로그 API 가 생기면)` 로 같이 걸어 뒀다.
