...
⚠️ **리그 화면과 종 상세의 슬롯 표는 「교체」 값이 다르다.** 원본이 표를 둘 그린 것이지
한쪽이 틀린 게 아니다.

| | 묻는 것 | 「몸」 |
|---|---|---|
| 리그 · 슬롯 | *리그가 무엇을 허용하는가* | 가능 |
| 종 상세 | *이 종이 무엇을 바꿀 수 있는가* | **고정** — 모든 종이 같은 몸을 쓴다 |

한쪽을 보고 다른 쪽을 고치면 조용히 어긋난다.
...
### 19.4.3 「덮어쓰기」 는 아이템 얘기다

⚠️ **몸 슬롯의 「덮어쓰기 불가」 를 「운영자가 못 고침」 으로 읽지 말 것.** 의상이 몸 위에
얹힐 뿐 몸 자체를 갈아끼우지 않는다는 뜻이고, **종의 기본 몸은 종마다 다르다** —
모루가 「통통한 몸」 을 쓴다. 그래서 여섯 슬롯 모두 편집할 수 있다.

둘을 헷갈려 몸을 읽기 전용으로 만들면 **모루의 값을 고칠 수 없게 된다.**

### 19.4.4 검사를 못 하는 상태 ≠ 통과한 상태
