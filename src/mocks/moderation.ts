...
/** 원본 `WHY` — 신고 폼의 선택지 순서 그대로. **마지막 「기타」 만 글을 따로 받는다** */
 ...
 * 「기타」 를 고른 사람이 직접 쓴 글.
 *
 * ⚠️ **길이를 일부러 들쭉날쭉하게 뒀다.** 앱은 500자까지 받으므로(`REASON_DETAIL_MAX`)
 *    짧은 것만 넣어 두면 **접기가 동작하는지 화면에서 확인할 수 없다** — 세 줄을 넘기는
 *    글이 하나는 있어야 한다 (docs/ARCHITECTURE.md §23.7).
 */
const DETAILS: string[] = [
  '이 사진 지난주에도 올라왔던 거예요. 같은 옷에 같은 배경인데 날짜만 바뀐 것 같습니다.',
  '인증 사진에 다른 사람 얼굴이 같이 찍혀 있습니다. 본인 동의를 받은 건지 모르겠어요.',
  '챌린지랑 상관없는 사진입니다.',
  '한 달째 같은 자리에서 같은 각도로 찍은 사진만 올라옵니다. 처음에는 그냥 습관인가 했는데, 지난 인증들을 쭉 넘겨 보니 컵에 담긴 물의 높이까지 똑같아서 같은 사진을 돌려 쓰는 것 같습니다. 확인 부탁드립니다.',
  '사진에 다른 앱 워터마크가 남아 있어요.',
]
...
// prettier-ignore
const ROWS: Row[] = [
  [0, '20:14', '주 3회 러닝', '새벽러너', '미검토', [2, 0, 1], [302, 1, 0, 0]],
  [0, '06:58', '아침 6시 기상', '민트초코', '미검토', [0, 1, 0, 2, 0], [41, 6, 2, 1]],
  [1, '22:41', '하루 한 챕터', '모카', '미검토', [3, 3, 1, 0], [55, 3, 1, 0]],
  [1, '07:02', '아침 6시 기상', '콩순이', '미검토', [1, 1, 0, 4, 2, 3], [8, 4, 3, 1]],
  [2, '19:30', '물 2L 마시기', '밤톨', '미검토', [4, 0, 1], [174, 1, 0, 0]],
  [3, '08:22', '스트레칭', '도토리', '숨김', [2, 2, 1, 0, 3, 4, 1], [213, 5, 4, 1]],
  [4, '11:20', '러닝', '민트초코', '노출 유지', [1, 0, 3, 2], [41, 6, 2, 1]],
  [5, '21:55', '독서', '해든', '노출 유지', [3, 4, 0], [89, 2, 0, 0]],
]
  ...
  return {
    ...
    reporters: why.map((w, k) => {
      ...
      return {
        ...
        why: reason,
        // 정해진 문구를 고른 사람에게는 글을 받는 칸 자체가 없다.
        detail: reason === REASON_WITH_DETAIL ? DETAILS[(key + k) % DETAILS.length]! : '',
      }
    }),
