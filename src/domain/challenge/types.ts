...
export type ChallengeReward = {
  /**
   * 가리키는 아이템 (`Item.key`). **상세를 열고 「지워졌는가」 를 물을 수 있는 유일한 값이다.**
   *
   * ⚠️ **`assetId` + 이름으로 아이템을 찾지 말 것.** §20.4 가 적어 둔 대로 **그림만 같고
   *    가격·획득 경로가 다른 아이템이 여럿일 수 있다** — 지금 목에 중복이 없을 뿐이다.
   *    같은 함정을 성장·둥지에서 이미 밟았다(§19.3.2).
   */
  itemKey: number
  assetId: string
