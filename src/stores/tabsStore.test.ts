...
const tab = (screen: string): OpenTab => ({
  screen: screen as OpenTab['screen'],
  path: `/${screen}`,
})
...
const dirtyOf =
  (...names: string[]) =>
  (t: OpenTab) =>
    names.includes(t.screen)
const clean = () => false

describe('evictionTarget', () => {
  it('깨끗한 것 중 가장 오래된 것을 고른다', () => {
    expect(evictionTarget(tabs('a', 'b', 'c'), 'c' as OpenTab['screen'], clean)?.screen).toBe(
      'a',
    )
  })
  ...
  it('⚠️ 전부 미저장이면 아무것도 고르지 않는다 — 상한보다 저장 안 된 내용이 우선이다', () => {
    expect(
      evictionTarget(tabs('a', 'b'), 'c' as OpenTab['screen'], dirtyOf('a', 'b')),
    ).toBeNull()
  })
