    ...
    <>
      ...
      <div className={css({ display: 'flex', flexWrap: 'wrap', gap: '18px', alignItems: 'flex-start' })}>
        <Card className={css({ flex: '1 1 340px', maxWidth: '440px', p: '15px 17px' })}>
          ...
          <div className={css({ display: 'flex', flexDirection: 'column', gap: '13px', mt: '13px' })}>
            ...
            {coin ? (
              ...
              <Select
                ...
                placeholder="고르세요"
                options={data.itemOptions.map((it) => ({ value: String(it.key), label: it.name }))}
          ...
          >
            <strong>되돌릴 수 없습니다.</strong> 관리자 지급분은 <strong>무상</strong>으로 적립되어
            환불 대상에서 빠집니다.
          </p>
        ...
        <div className={css({ flex: '2 1 460px', minWidth: '0' })}>
          ...
          <Table columns={COLUMNS} rows={data.logs} minWidth={880} rowKey={(g) => String(g.key)} />
        </div>
      </div>
