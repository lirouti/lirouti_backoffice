  ...
  return (
    <>
      ...
      <div className={css({ display: 'flex', flexWrap: 'wrap', gap: '18px', alignItems: 'flex-start' })}>
        <div className={css({ flex: '3 1 480px', minWidth: '0', display: 'flex', flexDirection: 'column', gap: '14px' })}>
          <Card className={css({ p: '18px 20px' })}>
            <CardTitle title="계정 정보" />
            <div className={css({ mt: '13px', display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: '12px' })}>
              <Input
                value={form.name}
            ...
            <div className={css({ mt: '14px' })}>
              <div className={css({ textStyle: 'label', fontWeight: '600', color: 'ink', mb: '7px' })}>역할</div>
              <Segmented
                value={form.role}
          ...
          <Card className={css({ p: '18px 20px' })}>
            ...
            <p className={css({ m: '7px 0 12px', textStyle: 'label', color: top ? 'sub' : 'sub' })}>
              {top
          ...
          <Card className={css({ p: '18px 20px' })}>
            <CardTitle title="보안" />
            <ul className={css({ m: '12px 0 0', p: '0', listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '9px' })}>
              ...
              <Note label="사내 IP만 허용">접속 대역을 서버가 아직 보지 않습니다. 준비 중입니다.</Note>
            </ul>
          </Card>
        ...
        <Card className={css({ flex: '1 1 300px', minWidth: '280px', p: '18px 20px' })}>
          ...
          <dl className={css({ m: '13px 0 0', display: 'flex', flexDirection: 'column', gap: '9px' })}>
            ...
            <Row
              k="담당 모듈"
              v={top ? `전체 ${ASSIGNABLE_SCOPES.length}개` : `${saved.scopes.length}개 선택`}
            />
            <Row k="첫 로그인 기한" v="발송 후 72시간" />
          </dl>

          <div className={css({ mt: '15px', pt: '13px', borderTop: '1px solid token(colors.ln)' })}>
            <div className={css({ textStyle: 'label', fontWeight: '700', color: 'ink', mb: '9px' })}>
              받는 사람이 하는 일
            </div>
            <ol className={css({ m: '0', p: '0', listStyle: 'none', display: 'flex', flexDirection: 'column', gap: '8px' })}>
              {STEPS.map(([n, label, note]) => (
                <li key={n} className={css({ display: 'flex', gap: '9px' })}>
                  ...
                  <span className={css({ minWidth: '0' })}>
                    ...
                    <span className={css({ display: 'block', textStyle: 'micro', color: 'sub' })}>{note}</span>
                  </span>
                </li>
              ))}
            </ol>
