    ...
    extend: {
      ...
      semanticTokens: {
        colors: {
           ...
           * 모달 뒤를 가리는 막.
           *
           * **결정을 요구하는 창**(`Dialog`)이 쓴다 — 뒤를 확실히 가려 지금 답할 것이
           * 하나임을 보인다. 라이트·다크가 같은 값이다: 하는 일이 「뒤를 어둡게」 라
           * 배경색이 무엇이든 방향이 같다.
           *
           * ⚠️ 원본에 없는 값이다(원본은 `.06`·`.14`·`.24` 만 쓴다) — 모달이 정적
           *    캔버스에 없었다. 두 곳이 `.42`/`.32` 로 갈려 있던 것을 토큰으로 올렸다
           *    (docs/ARCHITECTURE.md §62.2).
           */
          dim: { value: { base: 'rgba(16,24,40,.42)', _dark: 'rgba(16,24,40,.42)' } },
          /** 가볍게 떴다 사라지는 것(`CommandPalette`)의 막. `dim` 보다 옅다 */
          dimSoft: { value: { base: 'rgba(16,24,40,.32)', _dark: 'rgba(16,24,40,.32)' } },
      ...
      keyframes: {
        ...
        // 스켈레톤 막대의 숨. 화면을 통째로 채우는 자리에도 쓰이므로 **얕고 느리게** 둔다 —
        // 1.4s/0.55 로 두면 면적이 커졌을 때 화면이 깜빡이는 것처럼 보인다.
        rvPulse: {
          '0%, 100%': { opacity: '1' },
          '50%': { opacity: '0.72' },
        },
